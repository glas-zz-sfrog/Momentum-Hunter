...
class StorageSerializationTests(unittest.TestCase):
    ...
    def test_daily_capture_does_not_preserve_future_news_rows(self) -> None:
        ...
        self.assertEqual("morning", payload["capture_session"])
        self.assertEqual("MARKET_OPEN_DAY", payload["capture_calendar_status"])
        self.assertTrue(payload["is_market_open_day"])
        self.assertTrue(payload["is_study_eligible"])
        self.assertEqual("2026-06-04", payload["next_market_session_date"])
        self.assertEqual("market-calendar-v1", payload["scheduling_policy_version"])
