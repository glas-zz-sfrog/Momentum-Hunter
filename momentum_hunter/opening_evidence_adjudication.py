...
"""Write-once adjudication for captures invalidated by a known parser defect."""
...
def _render_markdown(payload: Mapping[str, object]) -> str:
    lines = [
        ...
        "",
        "| Date | Recorded candidates | Authority | Paper decisions |",
        "| --- | ---: | --- | ---: |",
    ]
    for case in payload.get("cases", []):
        ...
        lines.append(
            ...
            f"`{case['decisionState']}` | {len(case.get('paperDecisions', []))} |"
        )
    return "\n".join(lines)
...
def _case(value: str) -> OpeningEvidenceCase:
    ...
    if len(parts) != 6:
        raise argparse.ArgumentTypeError(
            "Case must be DATE|GIT_HEAD|CAPTURE|LOG|REPORT|PAPER_PATHS."
        )
    paper = tuple(Path(item) for item in parts[5].split(";") if item)
    return OpeningEvidenceCase(parts[0], parts[1], Path(parts[2]), Path(parts[3]), Path(parts[4]), paper)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = adjudicate_opening_evidence(args.case, repository_root=args.repository_root)
