...
HISTORICAL_QUOTE_PATH = re.compile(r"/v2/stocks/[A-Z][A-Z0-9.\-]{0,14}/quotes")
HISTORICAL_TRADE_PATH = re.compile(r"/v2/stocks/[A-Z][A-Z0-9.\-]{0,14}/trades")
...
def _require_allowed_path(path: str) -> None:
    if path in LATEST_PATHS.values() or any(
        pattern.fullmatch(path)
        for pattern in (
            HISTORICAL_BAR_PATH,
            HISTORICAL_QUOTE_PATH,
            HISTORICAL_TRADE_PATH,
        )
    ):
        return
...
def _symbols_from_params(params: Mapping[str, object], path: str) -> tuple[str, ...]:
    ...
    match = re.fullmatch(r"/v2/stocks/([^/]+)/(?:bars|quotes|trades)", path)
