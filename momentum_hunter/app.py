...
class MomentumHunterWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.gateway_page = self._build_gateway_page()
        self.steven_desk_page = self._build_steven_desk_page()
        self.argus_machine_page = self._build_argus_machine_console_page()
    ...
    def _build_gateway_page(self) -> QWidget:
        ...
        choice_layout.addWidget(
            self._build_gateway_choice(
                ...
                self.open_argus_machine_console,
            )
        )
    ...
    def _build_argus_machine_console_page(self) -> QWidget:
        page = QWidget()
        ...
        gateway_button.clicked.connect(self.show_gateway)
        ...
        desk_button.clicked.connect(self.open_steven_desk)
        header_layout.addWidget(gateway_button)
        header_layout.addWidget(desk_button)
        layout.addWidget(header_row)

        layout.addWidget(self._build_argus_machine_status_bar())
        ...
        left_layout.addWidget(self._build_argus_top5_panel())
        left_layout.addWidget(self._build_argus_workbench_panel())
        body.addWidget(left)
        ...
        right_layout.addWidget(self._build_argus_trade_plan_ladder_panel(), 2)
        right_layout.addWidget(self._build_argus_risk_governor_panel())
        right_layout.addWidget(self._build_argus_order_console_panel())
        body.addWidget(right)
        body.setStretchFactor(0, 2)
        body.setStretchFactor(1, 3)
        layout.addWidget(body, 1)

        layout.addWidget(self._build_argus_machine_log_panel())
        self._clear_argus_trade_plan_ladder()
        return page

    def _build_argus_machine_status_bar(self) -> QWidget:
        ...
        self.argus_machine_status_labels: dict[str, QLabel] = {}
        for column, (label, value) in enumerate(statuses):
            card = QLabel(f"{label}\n{value}")
            card.setObjectName("argusStatusCard")
            card.setWordWrap(True)
            layout.addWidget(card, 0, column)
            self.argus_machine_status_labels[label] = card
        return box

    def _build_argus_top5_panel(self) -> QWidget:
        ...
        self.argus_candidate_buttons: list[QPushButton] = []
        for candidate in ARGUS_MACHINE_PLACEHOLDER_CANDIDATES:
            ...
            button.clicked.connect(lambda _checked=False, item=candidate: self._select_argus_machine_candidate(item))
            layout.addWidget(button)
            self.argus_candidate_buttons.append(button)
        return box

    def _build_argus_workbench_panel(self) -> QWidget:
        box = QGroupBox("Selected Candidate Workbench")
        box.setObjectName("argusWorkbenchPanel")
        layout = QGridLayout(box)
        layout.setSpacing(8)

        self.argus_workbench_ticker_label = QLabel("Selected ticker: none")
        self.argus_workbench_ticker_label.setObjectName("argusWorkbenchTicker")
        self.argus_workbench_ticker_label.setWordWrap(True)
        self.argus_workbench_summary_label = QLabel("Setup summary: select a Top 5 placeholder candidate.")
        self.argus_workbench_summary_label.setWordWrap(True)
        self.argus_workbench_catalyst_label = QLabel("Catalyst placeholder: waiting for candidate selection.")
        self.argus_workbench_catalyst_label.setWordWrap(True)
        self.argus_workbench_chart_label = QLabel("Chart placeholder: waiting for candidate selection.")
        self.argus_workbench_chart_label.setWordWrap(True)
        self.argus_workbench_plan_status_label = QLabel("Plan status: no Trade Plan selected.")
        self.argus_workbench_plan_status_label.setWordWrap(True)

        labels = [
            self.argus_workbench_ticker_label,
            self.argus_workbench_summary_label,
            self.argus_workbench_catalyst_label,
            self.argus_workbench_chart_label,
            self.argus_workbench_plan_status_label,
        ]
    ...
    def _build_argus_trade_plan_ladder_panel(self) -> QWidget:
        box = QGroupBox("Trade Plan Ladder")
        box.setObjectName("argusTradePlanLadderPanel")
        layout = QVBoxLayout(box)
        layout.setSpacing(8)

        self.argus_ladder_empty_label = QLabel("Select a candidate to populate the Trade Plan Ladder")
        self.argus_ladder_empty_label.setObjectName("argusTradePlanEmptyState")
        self.argus_ladder_empty_label.setWordWrap(True)
        layout.addWidget(self.argus_ladder_empty_label)

        self.argus_ladder_table = QTableWidget(0, 2)
        self.argus_ladder_table.setObjectName("argusTradePlanLadderTable")
        self.argus_ladder_table.setHorizontalHeaderLabels(["Field", "Placeholder Value"])
        self.argus_ladder_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.argus_ladder_table.verticalHeader().setVisible(False)
        self.argus_ladder_table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
        self.argus_ladder_table.setSelectionBehavior(QTableWidget.SelectionBehavior.SelectRows)
        layout.addWidget(self.argus_ladder_table, 1)
        return box

    def _build_argus_risk_governor_panel(self) -> QWidget:
        ...
        self.argus_risk_gate_table = QTableWidget(6, 3)
        self.argus_risk_gate_table.setObjectName("argusRiskGateTable")
        self.argus_risk_gate_table.setHorizontalHeaderLabels(["Gate", "State", "Reason"])
        self.argus_risk_gate_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.argus_risk_gate_table.verticalHeader().setVisible(False)
        self.argus_risk_gate_table.setEditTriggers(QTableWidget.EditTrigger.NoEditTriggers)
        gates = [
        ...
        for row, values in enumerate(gates):
            for column, value in enumerate(values):
                self.argus_risk_gate_table.setItem(row, column, QTableWidgetItem(value))
        layout.addWidget(self.argus_risk_gate_table)
        return box

    def _build_argus_order_console_panel(self) -> QWidget:
        ...
        self.argus_preview_order_button = QPushButton("Preview Order")
        self.argus_submit_paper_button = QPushButton("Submit Paper Order")
        self.argus_submit_live_button = QPushButton("Submit Live Order")
        locked_buttons = [
            self.argus_preview_order_button,
            self.argus_submit_paper_button,
            self.argus_submit_live_button,
        ]
        ...
        self.argus_submit_live_button.setObjectName("argusSubmitLiveOrderButton")
        self.argus_submit_live_button.setToolTip("Locked: live trading requires separate Steven approval and broker safety gates.")
        layout.addWidget(button_row)
        return box

    def _build_argus_machine_log_panel(self) -> QWidget:
        box = QGroupBox("Machine Log")
        box.setObjectName("argusMachineLogPanel")
        layout = QVBoxLayout(box)
        self.argus_machine_log = QPlainTextEdit()
        self.argus_machine_log.setObjectName("argusMachineLog")
        self.argus_machine_log.setReadOnly(True)
        self.argus_machine_log.setMaximumHeight(96)
        self.argus_machine_log.setPlainText(
        ...
        layout.addWidget(self.argus_machine_log)
        return box

    def _clear_argus_trade_plan_ladder(self) -> None:
        self.argus_ladder_empty_label.setText("Select a candidate to populate the Trade Plan Ladder")
        self.argus_ladder_table.setRowCount(0)

    def _select_argus_machine_candidate(self, candidate: dict[str, str]) -> None:
        self.argus_workbench_ticker_label.setText(f"Selected ticker: {candidate['ticker']}")
        self.argus_workbench_summary_label.setText(f"Setup summary: {candidate['summary']}")
        self.argus_workbench_catalyst_label.setText(f"Catalyst placeholder: {candidate['catalyst']}")
        self.argus_workbench_chart_label.setText(f"Chart placeholder: {candidate['chart']}")
        self.argus_workbench_plan_status_label.setText(f"Plan status: {candidate['plan_status']}")
        self.argus_ladder_empty_label.setText(
        ...
        self.argus_ladder_table.setRowCount(len(rows))
        for row, (field, value) in enumerate(rows):
            self.argus_ladder_table.setItem(row, 0, QTableWidgetItem(field))
            self.argus_ladder_table.setItem(row, 1, QTableWidgetItem(value))
        self.argus_machine_log.appendPlainText(
