...
class WriterEnvelopeVerifier:
    ...
    def verify(self, envelope: WriterEnvelope) -> None:
        ...
        if envelope.configuration_fingerprint != self.configuration_fingerprint:
            raise WriterIpcError("Envelope configuration identity is invalid.")
        if envelope.source_identity != self.source_identity:
        ...
        expected_tag = _authentication_tag(bytes(self._key), envelope)
