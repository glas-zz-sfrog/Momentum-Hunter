...
This document names the current merged implementation paths on local `master` at `164e32e`, remote `master` at `69feedf`, and the explicitly labeled R027 Shadow plus Phase 12 integration candidate under review. The Roadmap, not this file, decides priority and next work.
...
| Research Maturity WPF evidence boundary | R027 integration candidate through R026/R025: `momentum_hunter/workstation_research_maturity.py`, `momentum_hunter/engine_host.py`, `src/MomentumHunter.EngineBridge/PythonResearchMaturityWorkspaceClient.cs`, `src/MomentumHunter.Presentation/ShellViewModel.cs`, `src/MomentumHunter.Desktop.Wpf/MainWindow.xaml` | Argument-free, read-only projection of persisted maturity/census evidence with separate denominators, strict strategy-lock validation, and no collection or action path. It becomes canonical only if Steven approves and merges R027. |
| Phase 12 integrated WPF candidate | Branch-only R027: `src/MomentumHunter.Desktop.Wpf/`, `src/MomentumHunter.Presentation/`, `src/MomentumHunter.EngineBridge/`, `src/MomentumHunter.Application/`, `src/MomentumHunter.Contracts/`, and the `momentum_hunter/workstation_*.py` projections | Verified two-parent integration of R013-R025 through R026 with the current Shadow baseline, using layout schema 7 and no rejected icon artwork. This is the sole current merge candidate; R026 and individual R013-R025 branches are preserved for audit only. |
...
| Python Engine Host | `momentum_hunter/engine_host.py` | Local loopback-only independent process. On `master` it exposes versioned lifecycle, persisted workspace, chart, FakeBroker-only simulation, and prospective Shadow capabilities. R027 preserves those and adds R026's technical-research, saved-watchlist, Daily Workflow, Candidate Story, and Research Maturity snapshots. Every new R026 command is read-only. It has no paper/live broker or real-order command. |
...
- Phases 8-10 plus R011/R012 are merged and backed up on remote `master`; local `master` additionally contains Shadow-001/002/003. R027 is the sole integrated Phase 12 plus Shadow merge candidate, while Paper and Live remain absent and locked.
- Do not merge R026 or R013-R025 individually while R027 is under review; retain them only as source/audit branches.
