...
| Future Schwab canary funding gate | `momentum_hunter/schwab_canary_funding.py` on `codex/ARGUS-CANARY-003-funding-restrictions` | Independent branch-only R-034 gate at `5702ad7`. It requires explicit settled-cash and restriction evidence and maps the current incomplete balance model to BLOCK. It becomes canonical only after clean integration. |
