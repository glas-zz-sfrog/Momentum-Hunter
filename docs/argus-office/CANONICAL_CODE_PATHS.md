...
| Future Schwab canary position invariant | `momentum_hunter/schwab_canary_positions.py` on `codex/ARGUS-CANARY-001-position-invariants` | Branch-only nontransmitting prerequisite at `7330983`. It consumes the existing GET-only adapter and evaluates exact zero/exact-canary/zero position states. It becomes canonical only after clean integration; no canary workflow or order path uses it yet. |
| Future Schwab canary position evidence | `momentum_hunter/schwab_canary_evidence.py` on `codex/ARGUS-CANARY-002-position-evidence-chain` | Stacked branch-only immutable chain at `f1b9042`. It binds and hash-links CANARY-001 results, records blocked attempts without advancement, and enforces PRE/ACTIVE/POST order. It becomes canonical only after clean stack integration; no generated evidence artifact is tracked. |
