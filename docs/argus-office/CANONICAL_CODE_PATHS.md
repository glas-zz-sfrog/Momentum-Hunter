...
| Unattended automation service | `momentum_hunter/automation_supervisor.py`, `src/MomentumHunter.AutomationService/`, and `tools/*automation_service*.ps1` | ARGUS-SERVICE-001 implementation path for boot-starting supervision, restart-safe one-time receipts, nonmarket account/DPAPI canary, guarded service installation/status, and downstream ephemeral read-only Codex review. It is not canonical until installed, verified, merged, and backed up; it has no real-order capability. |
