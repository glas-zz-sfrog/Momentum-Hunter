...
| Official Shadow evidence checkpoints | `momentum_hunter/shadow_evidence_checkpoints.py` and `momentum_hunter/workstation_shadow.py` on `codex/ARGUS-SHADOW-017-evidence-checkpoints` | Branch-only pending integration. Canonical-review-derived, immutable 5/10/20/30 JSON/Markdown checkpoints with hash-addressed trade manifests, automatic post-state invocation, exact-threshold preflight before further quote work, late-reconstruction metric withholding, source nonmutation, and no strategy/trading authority. |
