...
Backup state: local and remote `master` contain SHADOW-013 through `a39ad6b`. SHADOW-014 records the proof preparation facts before final artifact generation so the static bundle, exported one-time task definition, and inspection heartbeat can bind to the final synchronized governance commit rather than become stale immediately.
...
| Active branch | `codex/ARGUS-SHADOW-014-opening-proof-preparation`; governance-only checkpoint before final-head artifact regeneration and one-time scheduling. |
...
| Immediate operational work | Integrate and back up this governance checkpoint, regenerate the static bundle and one-time task XML from that final head, schedule the proof-only task for 2026-07-28 at 8:35 AM CT, and run one finite read-only 8:50 audit. Do not pass `-ArmShadowSelector`; do not synthesize or backfill Trade 1. |
