...
Backup state: local and remote `master` are synchronized at SHADOW-014 scheduling commit `4c35181`. The static bundle, exported one-time task definition, and inspection heartbeat bind to that exact canonical commit. Separately, R030-R034 are implemented and verified through R034 implementation commit `a882463`; SHADOW-015 evidence is preserved on the stacked successor branch and externally under `ArgusReviewBundles`. The stack remains unmerged so the scheduled proof baseline cannot move.
...
| Active branch | `codex/ARGUS-SHADOW-015-opening-failure-rehearsals`, stacked on verified R030-R034 through `972b5d9`. SHADOW-015 is evidence-only; the complete stack remains `IMPLEMENTED_PENDING_MERGE` and will be backed up without moving canonical `master`. |
...
| Immediate operational work | Allow the one-time proof-only task and read-only heartbeat to run. Do not manually launch, retry, repair, arm, select, or synthesize/backfill Trade 1. After its audit, fast-forward the verified R030-R034 stack, run the write-once full-target cutover preflight from the integrated commit, then stop at the explicit destructive cutover decision. |
