...
| Item | Current truth |
| --- | --- |
| Canonical baseline | `master` contains the WPF workstation through R029, Python engine contracts, Shadow-001 through Shadow-011, and SCHWAB-001/002/002A/003 read-only safeguards. SHADOW-011 makes SHADOW-010's automatic proof ceremony evaluate quotes against a truthful post-response clock. |
| Active branch | No implementation branch is active. `codex/ARGUS-SHADOW-011-proof-timestamp-ordering` and earlier Shadow branches are preserved as merged source history. |
