...
| Active branch | `codex/ARGUS-R035-candle-input-reconcile-1af5b31` is the isolated verified product lane. `codex/ARGUS-SHADOW-016-017-reconcile-1af5b31` separately preserves verified post-opening evidence work; neither branch moves the frozen scheduled checkout before the operational audit. |
...
| Immediate operational work | Preserve the final-head static bundle and exact installed-task definition, then inspect the one-time 2026-07-29 opening after its finite runner completes. Do not launch it early, retry a terminal failure, reuse expired proof, synthesize/backfill Trade 1, or bypass fresh quote/clock validation. Keep the verified R035 and SHADOW-017 feature branches backed up; integrate them only after the frozen operational audit releases `master`. |
...
Status: `ACTIVE`; R011-R029 plus Shadow-001/002/003 are `COMPLETE` on local `master`; R035 is `IMPLEMENTED_PENDING_MERGE`; remaining Qt retirement stays incremental
