...
| Active branch | `codex/ARGUS-SHADOW-016-paper-reconciliation-evidence` at `6c1fc7b`; nonvisual manual paperMoney reconciliation evidence is implemented and verified pending governance closeout and branch backup. |
...
| Immediate operational work | Allow the one-time proof-only task and read-only heartbeat to run without manual launch, retry, repair, arm, selection, or backfill. In parallel, finish and integrate independent SHADOW-016 reconciliation evidence and R035 candle-input hardening only after the proof-bound baseline is released. |
...
- SHADOW-016 implements that manual bridge as a separate append-only JSON record bound to the exact frozen Shadow state hash, order, TradePlan, Risk Governor decision, evidence snapshot, sample definition, and selector/cycle identity. It requires an operator-entered ticket and explicit terminal paperMoney result, validates fill/result chronology and consistency, is idempotent only for an exact duplicate, rejects conflicting or tampered evidence, and never mutates the source state. It remains branch-only at `6c1fc7b` until the frozen SHADOW-014 proof baseline is released.
