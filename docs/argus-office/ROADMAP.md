...
Last reconciled: 2026-07-25 on `codex/ARGUS-SCHWAB-002A-credential-rotation` after a post-integration credential-exposure stop. SCHWAB-002 remains `COMPLETE` on local `master` and unpushed, but its current local Client Secret and OAuth material are quarantined and must not be used. During read-only research in Schwab's official developer portal, an expired-session page retained the live Client ID and Client Secret in its DOM; browser inspection surfaced those values to the automation channel. The values are not repeated in any report, and an exact in-memory scan against every Git-tracked file found zero Client ID hits and zero Client Secret hits. No account, balance, position, market-data, preview, or order request occurred; `accountBinding` remains `NOT_BOUND`, authenticated account requests remain locked, and order transmission remains unavailable. The required recovery is a separately confirmed Schwab Client Secret rotation, exact local-auth deletion, hidden entry of the replacement credentials, and fresh OAuth consent limited to the intended $100 account. Account discovery cannot begin until the replacement chain is verified. SCHWAB-002's compileall, focused 60/60, and full 702/702 evidence remains valid for the implementation. No scoring, readiness, replay, alert, database, schema, market-data, TradePlan, Risk Governor, broker/order, WPF, generated-data, or sample behavior changed. Only `CRWV` has stored minute candles; the future actual-data cutover purge gate remains mandatory. The official evidence sample has not started; `SAMPLE START LOCKED` remains canonical.
...
| Integrated implementation | Phases 8, 9, and 10, R011-R029, ARGUS-SHADOW-001/002/003, SCHWAB-001, and SCHWAB-002 are `COMPLETE` on local `master`. SCHWAB-002 is not yet pushed, and its current credential/token material is quarantined pending rotation. Phase 11 remains `ACTIVE` because the official Shadow sample is locked and authenticated account discovery, exact account binding, broker reads, and every order gate remain not started. A017's automated Schwab paper API path remains `BLOCKED_VENDOR_CAPABILITY`. |
...
| Immediate next action | Rotate the exposed Schwab Client Secret, delete the quarantined local auth store, enter the replacement credentials through hidden prompts, and repeat OAuth for only the intended $100 account. Each destructive or persistent-access action requires Steven's explicit confirmation. Only after recovery passes may Steven separately authorize read-only account discovery and exact single-canary binding. Pushing local `master` and starting the official Shadow sample remain separate approvals. |
...
| Broker and execution state | FakeBroker remains the only automated execution boundary. Schwab `Trader API - Individual` access is approved, and production app `Market Intelligence Workstation` is `Ready For Use` with `Accounts and Trading Production`, `Market Data Production`, callback `https://127.0.0.1:8182/oauth/callback`, and an order-request throttle of `5` per account per minute. Production-local certificate version `20260725T004100Z-feaa7bc59097` remains trusted only in `CurrentUser\Root` and browser-verified. SCHWAB-002 stores credentials/tokens in current-user DPAPI ciphertext outside Git, but the current Client Secret appeared in browser-automation output during official-portal research and is quarantined pending rotation. Exact tracked-file scanning proves neither the Client ID nor Client Secret exists in Git. No account hash or account binding exists, no authenticated account request has occurred, and no paper/live broker or transmitting method exists. Paper and Live remain software-locked. |
...
Status: `ACTIVE`; ARGUS-SHADOW-001/002/003, R027, SCHWAB-001, and SCHWAB-002 are `COMPLETE` on local `master`; SCHWAB-002 remains unpushed; current Schwab credential/token material is `QUARANTINED_ROTATION_REQUIRED`; the official sample is `NOT_STARTED`; Schwab A017 is `BLOCKED_VENDOR_CAPABILITY`; production-local certificate trust is `PASS`; account discovery, exact account binding, authenticated broker reads, and broker/order paths remain separately gated and not started
...
- Post-integration official-portal research exposed the current Client Secret to the browser-automation channel because Schwab's expired-session DOM retained the revealed value. Treat the credential and derived OAuth material as compromised. Exact in-memory comparison against every tracked file proves zero Client ID and zero Client Secret hits in Git. Do not use or refresh the current token; rotate the Client Secret and replace local auth before any account request.
- Schwab's public `Authenticate with OAuth` guide confirms that Consent and Grant lets the user select which accounts are shared. Its public `OAuth Restart vs. Refresh Token` guide says a user can revoke token access at any time, revocation should terminate third-party access unless explicitly granted again, a compromised refresh token requires the full OAuth flow, and a different authorized-account selection requires a new access token. Recovery therefore requires remote revocation where available, Client Secret rotation, local-auth deletion, and a complete new consent flow rather than token refresh.
- Steven approved credential recovery. Schwab Security Settings showed that `Market Intelligence Workstation` was linked only to the intended Individual account while the Rollover IRA and Joint Tenant accounts were unchecked. `Stop Linking` was confirmed; the workstation entry disappeared without changing unrelated linked apps. The guarded local-auth deletion then produced `credentialsStored: false`, `oauthAuthorized: false`, `tokenState: MISSING`, `accountBinding: NOT_BOUND`, authenticated account requests locked, and order transmission unavailable.
- The compromised developer app is now `Deactivated`. Schwab exposes no Client Secret rotation control: its official guide says Modify App changes app metadata/callbacks, while activation/deactivation only pauses or resumes the same app. A replacement app with the same two production products is blocked because Individual access allows one app per product and the deactivated app still consumes both slots. The old app also cannot remove its final API product, so self-service credential replacement is exhausted. The next required action is Schwab Trader API Support resetting the inactive app's credentials or deleting it so a replacement can be created. No credential, token, account, market-data, preview, or order request may occur before that provider action and a complete fresh OAuth consent.
