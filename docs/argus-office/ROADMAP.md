...
ARGUS-SHADOW-025K writer-boundary architecture is
`IMPLEMENTED_PENDING_MERGE` on
`codex/ARGUS-SHADOW-025K-writer-boundary-contract`, stacked on verified 025J
head `1015bbd`. A dormant fingerprinted contract now rejects the current same-
SID service/Engine Host/WPF arrangement and evaluates two prospective security
boundaries: a distinct-principal Engine Host that owns the evidence root, or a
distinct evidence-writer process reached through a nonpersistent authenticated
capability. It never creates a principal, reads or moves a credential, opens a
channel, changes an ACL, installs a process, or authorizes activation.

The contract binds the chosen proposal to 025J's root-security policy, keeps
Automation Service supervisor-only and WPF outside the raw root, rejects
multiple writers, same-SID writer aliases, forgeable same-user named-pipe
authority, persisted or interactive-visible capabilities, and provider-secret
leakage. Current-user DPAPI cannot follow a distinct Engine Host silently;
that route requires separately approved reprovisioning or an independently
authenticated ephemeral credential broker. Compileall, 22 focused tests, 196
adjacent runtime tests, and all 1,871 Python tests pass. Canonical `master`
remains clean and
synchronized at frozen `78db1bf`; merge/install waits for Tuesday's terminal
evidence. Selecting and physically proving one boundary remains prospective.

ARGUS-SHADOW-025J runtime-root security is `IMPLEMENTED_PENDING_MERGE` on
`codex/ARGUS-SHADOW-025J-runtime-root-security`, stacked on verified 025I head
`dce582c`. A dormant contract now evaluates complete fingerprinted Windows
path-component, ownership, reparse, DACL, and effective-access evidence before
a future installed continuous-evidence root can be considered contract-
eligible. It always leaves activation unauthorized and has no inspector,
installer, ACL mutation, root creation, or runtime import capability.

The contract requires one writer SID distinct from interactive/WPF and broad
principals, trusted nonwriter ownership, a protected root DACL, least-privilege
writer append/atomic-replace rights, no nonwriter root mutation, and no
destructive ancestor access. Source inspection confirms the current service is
installed under the current user and launches Engine Host without an alternate
username, so Engine Host and interactive WPF presently share a SID and cannot
claim ACL process-role isolation. Compileall, 19 focused, 145 adjacent runtime,
and all 1,849 Python tests pass. Canonical `master` remains clean and
synchronized at frozen `78db1bf`; merge/install waits for Tuesday's terminal
evidence. A distinct writer security boundary, elevated read-only ACL
inspection, safe provisioning, TOCTOU proof, and actual restart proof remain
prospective work.

ARGUS-SHADOW-025I runtime orchestration is `IMPLEMENTED_PENDING_MERGE` on
`codex/ARGUS-SHADOW-025I-runtime-orchestration`, stacked on verified 025H head
`4e61865`. One dormant transaction now accepts an already-built candidate,
plan, source-admission, policy, and decision bundle, acquires the current
topology-bound sole-writer lease, previews the proposed full chain in memory,
and replays all four persistence stages in fixed order. It creates no evidence
input and has no installed importer or external capability.

Partial candidate batches plus candidate-only, plan-only, and admission-only
crash prefixes resume
idempotently. A complete exact replay changes no evidence byte; invalid plan/
decision binding, chronology, configuration, result fingerprint, or writer
identity fails closed. The after-snapshot proves the exact target completed
without hiding unrelated pending plans. Compileall, 14 focused, 126 adjacent
runtime, 266 broader boundary, and all 1,830 Python tests pass. Canonical
`master` remains clean and synchronized at frozen `78db1bf`; merge/install
waits for Tuesday's terminal evidence. Installed-root selection, ACL/reparse/
raw-store confinement, actual Engine Host import, and production restart proof
remain separate prospective work.

ARGUS-SHADOW-025H runtime recovery planning is
`IMPLEMENTED_PENDING_MERGE` on
`codex/ARGUS-SHADOW-025H-runtime-recovery-planner`, stacked on verified 025G
head `576aef9`. A dormant read-only planner now validates all four ledgers plus
their candidate -> plan -> source-admission -> receipt/cycle bindings and
classifies exact valid prefixes as empty, waiting, resumable at one or multiple
stages, or complete. Its fingerprinted snapshot names pending/completed IDs and
artifact hashes but has no method that performs a recovery action.

Malformed, cross-program, orphaned, raw-store-bypassed, or directly
handcrafted unvalidated evidence fails closed. Pre/post inspection hashes reject
concurrent evidence changes, repeated inspection is deterministic, and source
bytes remain unchanged. Compileall, 19 focused recovery, 47 combined session/
chain/recovery, 281 runtime-boundary, and all 1,816 Python tests pass. No
existing runtime imports the planner; canonical `master` remains clean and
synchronized at frozen `78db1bf`. Merge/install waits for Tuesday's terminal
evidence. A later orchestrator must still revalidate writer authority before an
append; installed-root selection, ACL/reparse/raw-store confinement, and actual
Engine Host recovery remain separate prospective work.
...
ARGUS-SHADOW-025B head `6ba54bd`. The dormant continuous-decision path now
admits exactly one canonical source for each new plan version: a setup-bound
candidate lifecycle event when candidate identity changes, or the exact
validated successor plan after regime, catalyst, RVOL, clock, setup, or other
context changes. Discovery-only, unpersisted, contradictory, nonmaterial, or
reused-identity sources fail closed, and evidence refresh cannot double-fire a
candidate event plus a plan event.

Compileall, 59 focused source/event-cycle tests, 115 direct contracts, 285
combined continuous tests, and 130 allocation/Paper/Shadow/candle regressions
pass; all 1,734 Python tests pass. The adapter has no network, provider,
account, broker, order, persistence, service, scheduler, Engine Host, WPF, or
installed runtime capability. Canonical `master` remains frozen at `78db1bf`;
merge/install waits for Tuesday's terminal opening and Paper evidence.
Installed source paths, process ownership, and orchestration remain a separate
prospective task.

ARGUS-SHADOW-025B cross-process ledger ownership is
`IMPLEMENTED_PENDING_MERGE` on
`codex/ARGUS-SHADOW-025B-cross-process-ledger-ownership`, stacked on the
verified ARGUS-CONTINUOUS-002 head `657cb37`. The dormant event-cycle store now
owns one deterministic sidecar lease per resolved ledger path, serializing the
complete read/validate/append/atomic-write transaction across Windows processes
with a finite configurable timeout. Same-thread nested transactions remain
reentrant, direct saves use the same lease, and an exited process releases OS
ownership without deleting or racing the sidecar.

Compileall, 42 focused event-cycle tests, 98 direct contract tests, 268 combined
continuous tests, 130 allocation/Paper/Shadow/candle regressions, and all 1,717
Python tests pass. Synthetic Windows-spawn proof covers simultaneous writers,
finite contention timeout, recovery after release, and recovery after a process
exits while holding the lease. No existing runtime imports the module; no
provider, account, broker, order, service, scheduler, Engine Host, WPF,
production store, Paper, or Shadow state was contacted or changed. Canonical
`master` remains frozen at `78db1bf`; merge/install waits for Tuesday's terminal
opening and Paper evidence.

ARGUS-CONTINUOUS-002 offline contract reconciliation is
`IMPLEMENTED_PENDING_MERGE` on
`codex/ARGUS-CONTINUOUS-002-offline-contract-reconciliation`, based on the
verified common continuous head `f6b776e`. It combines the hardened PLAN-002A,
BREAKOUT-002A, and SHADOW-025A contracts in dependency order without touching
canonical `master` or Tuesday's installed runtime. The reconciliation found two
cross-branch defects: SHADOW-025A's synthetic factory used the older PLAN schema,
and a rehashed event-cycle decision could copy plan authority facts that did not
match its exact supplied plan. The fixture now uses the prospective-only schema,
and event-cycle validation independently binds plan status/blockers before any
cycle can be persisted.

Compileall, 94 focused PLAN/outcome/event-cycle tests, 264 combined continuous
contract tests, 130 adjacent DATA-004/RVOL/allocation/Paper/Shadow/candle tests,
and all 1,713 Python tests pass. The modules remain dormant and have no existing
runtime importer, provider/account access, broker/order capability, production
store, service, scheduler, Engine Host, WPF, score, readiness, selector, Risk
Governor, allocation-policy, Shadow, or Paper-state effect. Canonical `master`
remains clean and synchronized at `78db1bf`; merge/install waits for Tuesday's
terminal opening and prospective Paper engineering evidence.
...
verified ARGUS-CONTINUOUS-001 feature head `f6b776e`. It adds a dormant,
provider-neutral envelope that binds one immutable DATA-004 IntradayPlan to the
exact candidate opportunity/setup revision, regime snapshot, macro-event
...
service, and Tuesday's opening/Paper jobs remain unchanged.
...
head `06e1ea7`, based on synchronized canonical `78db1bf`. It reconciles the
previously proven candidate-lifecycle, rolling market/sector regime, versioned
macro-event context, provider-neutral catalyst evidence, and sequential
breakout-research modules onto the current Alpaca Paper engineering baseline.
The modules remain dormant evidence contracts: no existing runtime imports
them, and no service, Engine Host, scheduler, WPF, score, readiness, selector,
TradePlan, Risk Governor, account, broker/order, Shadow, or production-store
path was changed. Compileall, 160 focused tests, 250 adjacent candle/plan/
allocation/Paper/automation regressions, and all 1,619 Python tests pass. Static
boundary, protected-path, secret, and whitespace scans pass. Canonical `master`,
the installed runtime, and Tuesday's pinned opening/Paper jobs remain unchanged;
integration waits until their terminal evidence is preserved.

ARGUS-BROKER-ALPACA-004 Paper engineering wiring is integrated and backed up
through implementation commit `93f944c`. The running Automation Service
hot-loaded the new canonical code, the canary engineering sample
`alpaca-paper-engineering-20260810-v1` is frozen, and one Paper job is `PENDING`
for Tuesday August 11 at 08:35 Central behind that day's opening capture. The new
path is Schwab trusted evidence -> DATA-004 TradePlan -> Paper Risk Governor ->
DATA-005B provider-neutral allocation -> Canary Alpaca Paper -> broker-resident
fractional stop -> bounded reconciliation -> immutable decision/outcome. Every
cycle terminates as `PAPER_TRADE_CREATED` or truthful `NO_TRADE`; it is
prospective, Paper-only, and excluded from the final continuous-intraday
30-trade strategy sample.
...
automation tests, and all 1,459 Python tests pass. Activation preflight reached
only the exact Alpaca Paper host and found the Canary account `ACTIVE` with
`$100` cash/buying power, zero positions, and zero open orders; it made zero
mutations. No Paper order was made by this implementation. No live Alpaca host, Schwab
order, Shadow activation, score/rank/alert change, database/schema change, UI
change, or final-sample claim exists. Immediate next action is let the independent
service complete Tuesday's opening capture, then preserve the first prospective
`PAPER_TRADE_CREATED` or `NO_TRADE` result. The final closeout head must remain
clean, synchronized, and identically pinned on both Tuesday jobs.
...
| Active implementation | Eight consecutive unattended captures through Monday August 10 passed. DATA-005A, the directly proven A001-A003 Paper foundation, DATA-005B provider-neutral allocation/Paper-research contracts, and ARGUS-BROKER-ALPACA-004 Paper engineering wiring are complete on canonical `master`. The stacked ARGUS-SHADOW-025A through 025J feature branches now extend the reconciled dormant continuous/PLAN/BREAKOUT/event-cycle stack through cross-process ownership, exact source admission, topology, persistence, process-lifetime writer authority, four-stage evidence composition, read-only recovery planning, idempotent prevalidated orchestration, and a fail-closed installed-root security contract; none of that stack is installed. The running service has the Paper job pending; the first prospective decision is next. R034 remains a separate destructive approval gate. |
...
| Blocked by | The first prospective Paper engineering decision is waiting only for the next fresh opening capture and market evidence; no code or policy blocker remains. The stacked SHADOW-025 implementation is not blocked, but canonical merge/install waits for Tuesday's terminal opening/Paper evidence so the pinned runtime is not changed overnight. PLAN-002 production authority remains gated on integrated prerequisites and frozen live catalyst/macro/setup policy identities; BREAKOUT-002 conclusions require a frozen sufficient prospective cohort. SHADOW-025 source admission, cross-process ownership, topology, persistence, writer session, evidence chain, recovery planning, orchestration, root-security policy, and writer-boundary alternatives are synthetically proven. Installed activation remains ineligible until one distinct writer architecture is selected and physically proved; elevated ACL inspection/provisioning, credential/channel proof, startup import, and production recovery remain unwired. Partial-fill and provider-restart behavior remain synthetic-only; broker-resident linked protection and status streaming remain separate unknowns. DATA-002 authority remains fail-closed on incomplete current-window/baseline bars; DATA-004 requires real chronology and successor identity for reclaim. R034 remains separately destructive approval-gated. Fully powered-off recovery still depends on BIOS RTC/restore-on-AC-loss. |
...
| Immediate operational work | Preserve Tuesday's opening receipt, then adjudicate and preserve the first prospective Paper engineering `PAPER_TRADE_CREATED` or `NO_TRADE` result. After that evidence is terminal, reconcile the stacked ARGUS-SHADOW-025A through 025K chain against the then-current canonical base and serialize it into the next integration window. Select and physically prove either the distinct-principal Engine Host or dedicated evidence-writer boundary before SHADOW-025 installed-root creation/import; credential movement or reprovisioning remains a separate consequential decision. Runtime PLAN-002 and BREAKOUT-002 cohort activation remain separate prospective tasks. Present R034's exact deletion plan only when Steven is ready. |
...
- ARGUS-SHADOW-024 is implemented, Hard-Chew verified, integrated, and backed up
  ...
  prerequisite passed.
- ARGUS-SHADOW-025B is implemented and Hard-Chew verified on an isolated
  feature branch. It upgrades SHADOW-025A's explicit-path event-cycle ledger
  from in-process serialization to one finite, reentrant, cross-process file
  lease per resolved path. Windows-spawn tests prove no lost concurrent append,
  bounded timeout, ordinary recovery, and OS release after process exit. The
  contract remains dormant; runtime source/path selection and installation are
  separate prospective work after Tuesday evidence.
- ARGUS-SHADOW-025C is implemented on the successor feature branch. It admits
  one exact setup-bound candidate event or one exact successor plan as the sole
  source of each dormant decision trigger. Canonical-ledger membership,
  predecessor chronology, materiality, policy configuration, setup binding,
  and source fingerprints fail closed; context changes and candidate evidence
