...
Last reconciled: 2026-07-26 on `master`. The complete SHADOW-004 through SHADOW-008 stack is fast-forward integrated and backed up through `fdcf898`; Steven's SHADOW-004 visual acceptance remains the canonical UI proof. The runtime uses an immutable write-once arm record tied to 12 structured prerequisite artifacts, hash-verified evidence, the exact activation, the constitution hash, and a Shadow runtime build hash. `selector-arm-check` verifies the complete bundle without persistence; the separate exact-phrase `selector-arm` command creates only policy and arm state after the same verification and exposes no broker call. SHADOW-008 adds the missing production bundle assembler: `prepare-static` requires clean synchronized `master`, required commit ancestry, retained visual acceptance, and 28 named focused tests before atomically writing 11 static proofs; `finalize` accepts only an exact candidate-plus-SPY/IWM proof stamped by the normal Schwab CLI transport, rechecks the 30-second regular-market boundary and every static hash, appends the twelfth proof, and runs the canonical arm verifier without arming. Injected, stale, mismatched, tampered, partial, duplicate, dirty-branch, or changed-build evidence fails closed and removes temporary fresh artifacts. Canonical selection continues to include rank/score/identity ordering, warning severity, report/provider/quote identity, Opportunity ID and same-day duplicate controls, one global active position, session/early-close/no-overnight behavior, expected-cycle and restart-downtime accounting, SPY/IWM and eligible-candidate counterfactuals, and the 10-session strategy-review gate. A live Sunday read proved the real Schwab response shape and correctly rejected Friday CRWV/SPY quotes as stale, closed, extended-hours evidence; it does not satisfy the pending open-market proof. Compileall, 26 focused builder tests, all 28 named proof-gate tests, 123 adjacent Shadow/Engine Host tests, full Python discovery at 854/854, and all 216 .NET tests pass. Production-local proof keeps activation SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`, arm/policy/cycle/state absent, and the official sample at `0 / 30`. FakeBroker remains the only automated execution boundary; Schwab remains read-only and bound to the sole `2573` `CASH` account.

Backup state: local and remote `master` contain the integrated SHADOW-008 implementation through `fdcf898`; this reconciliation will be backed up before the canonical ignored static bundle is prepared.
...
SHADOW-008 proof-bundle assembly is integrated and backed up at `fdcf898`. Quote-proof schema v2 distinguishes `LIVE_SCHWAB_TRADER_API`, `INJECTED_SOURCE`, and unspecified sources; only the normal CLI-created Schwab transport path is marked production. The new nontransmitting assembler creates 11 atomic static proof artifacts on synchronized canonical `master` and can append the live quote boundary only when exact current candidate, SPY, and IWM evidence passes. It never calls `selector-arm`, writes policy, creates a cycle or trade, or exposes an order endpoint. The ignored production bundle is prepared as the final post-reconciliation operation so its Git/build evidence matches the backed-up `master`.

| Item | Current truth |
| --- | --- |
| Canonical baseline | `master` contains the WPF workstation through R029, Python engine contracts, Shadow-001 through Shadow-008, and SCHWAB-001/002/002A/003 read-only safeguards. Integration anchor `307a2e1` includes the accepted visual proof; `79e75b2` makes pre-arm status unambiguous; `fdcf898` supplies the fail-closed production proof assembler. |
| Active branch | No implementation branch is active. `codex/ARGUS-SHADOW-008-proof-bundle-assembly` and earlier Shadow branches are preserved as merged source history. |
...
| Blocked by | One regular-market operational proof that Schwab provides the current candidate plus SPY/IWM as tradable quotes inside the frozen 30-second boundary. The static 11-of-12 bundle can be prepared beforehand; the live quote is the only remaining artifact. |
| Immediate Builder work | During the next regular market, identify the current report candidate, run the read-only Schwab proof for that exact symbol plus SPY/IWM, finalize the existing static bundle, run `selector-arm-check`, and create the write-once arm only if every verifier returns `PASS`. Keep the official sample at zero until then. |
...
| Data caveat | Legacy/current persisted bid/ask rows with only monitor-cycle timestamps remain unavailable rather than presumed fresh. The new Schwab source is read-only and provider-timestamped, but weekend/closed/extended/stale quotes still produce no eligible candidate; regular-market latency proof remains pending. Only `CRWV` has stored minute candles; actual-data cutover remains a destructive-operation interruption gate. The frozen early-close table covers 2026-2028 and fails closed beyond it. |
...
Status: `ACTIVE`; SHADOW-001 through SHADOW-008 and the Schwab read-only foundations are `COMPLETE` on `master`; the official sample is `ACTIVATED` with `SELECTOR_NOT_ARMED` and `0 / 30` completed trades; the regular-market quote proof is pending; A017 is `BLOCKED_VENDOR_CAPABILITY`; every real-order gate remains closed
