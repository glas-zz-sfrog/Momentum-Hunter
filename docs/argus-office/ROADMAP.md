...
| Active branch | None after clean SHADOW-016 integration; implementation evidence remains on `codex/ARGUS-SHADOW-016-one-time-arm-scheduler`. |
...
| Immediate operational work | Preserve the final-head static bundle and exact installed-task definition, then inspect the one-time 2026-07-29 opening after its finite runner completes. Do not launch it early, retry a terminal failure, reuse expired proof, synthesize/backfill Trade 1, or bypass fresh quote/clock validation. Independent engine, research, and workstation work may continue in parallel. |
