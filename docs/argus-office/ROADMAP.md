...
Last reconciled: 2026-07-24 during Steven's manual review and repair of R027 on `codex/ARGUS-R027-integrate-r026-with-shadow-baseline`, the Schwab application-registration checkpoint, and credential-free loopback listener/certificate-lifecycle implementation on stacked branch `codex/ARGUS-SCHWAB-001-loopback-oauth-listener`. Local `master` remains unchanged at `164e32e`, ten commits ahead of `origin/master` at `69feedf`; nothing was pushed. Safety branch `safety/ARGUS-R027-before-r026-integration` preserves that exact master. R027 preserves a real two-parent integration of master `164e32e` and R026 `838ed22` without rebasing either history. Runtime conflicts preserve Shadow review/host behavior alongside every R013-R025 read-only workstation capability. The original integrated branch passed Python compileall, 146 focused Python tests, full Python discovery at 641/641, all 206 .NET tests, a zero-warning Release build, protected-path review, source-nonmutation tests, and fresh offscreen UI proof. Manual review confirms the required `CRWV` / `5m` stored-candle case, hover inspection, pane hide/reopen behavior, prohibited-action absence, rejected-artwork absence, and a provisional pass for the current Trade Plan `Plan`, `Why`, `Research`, and `History` surfaces. Their deeper evidence-quality review is deferred until broader market data exists; `CRWV` alone is not treated as proof of broader quality. Manual review found the top `Daily` control clipped, `Link A` / `Link B` meaningless, pane and toolbar icon actions oversized or clipped, missing hover help, destructive pane removal without an add-back route, a Command Palette that implied broader symbol search than it supported, and jargon-heavy repeated safety labels. Committed repair `f84106a` restores and focuses panes consistently, removes destructive pane deletion from the normal menu, uses compact stateful pane actions and centered toolbar icons with explicit tooltips, restores missing standard panes from saved layouts, uses operator-facing sync labels, labels the current-evidence workspace `Current`, makes palette scope and misses explicit, displays `Test Trade Review`, consolidates mode warning into the single top badge, and removes duplicate toolbar/status labels. Presentation tests pass 162/162, the complete .NET solution passes 209/209, and the Release build has zero warnings. CLI-sized proof at `1180 x 820` and `1440 x 900` confirms the full toolbar remains visible, while separate rendered proof confirms compact pane actions. A stale July 23 Python host was gracefully replaced after it exposed an `UNSUPPORTED_COMMAND` mismatch; the current host and UI now agree that sample start is locked, all counts are zero, the gate is `0 / 30`, and aggregate metrics are withheld. Because there are zero test trades, selection and populated-filter checks are honestly `MANUAL_NOT_YET_AVAILABLE`. R027 remains `IMPLEMENTED_PENDING_MERGE` and is not merge-ready until Steven physically reverifies the remaining visual repairs. The stacked SCHWAB-001 branch implements a one-use HTTPS callback receiver plus a versioned Windows certificate lifecycle with encrypted leaf keys, DPAPI password storage, explicit ACL verification, and reversible trust contracts. All certificate proof used temporary material: no production certificate was staged, no root was installed, no credential was used, no token was exchanged, no account was accessed, Schwab was not contacted, and no order path was created. TEST-001's identical Qt fixes are included through R026 and its separate branch is superseded. The official evidence sample has not started; `SAMPLE START LOCKED` remains canonical.

| Item | Current truth |
| --- | --- |
| Canonical product baseline | Local `master` contains the Phase 10 foundation, R011/R012 chart work, ARGUS-SHADOW-001 prospective Shadow lifecycle, ARGUS-SHADOW-002 read-only WPF review, and ARGUS-SHADOW-003 sample-readiness gate. Remote `master` remains at `69feedf`. |
...
| Integrated implementation | Phases 8, 9, and 10 plus R011/R012 are `COMPLETE` on local and remote `master`. ARGUS-SHADOW-001/002/003 are `COMPLETE` on local `master`. R013-R025 and the Shadow baseline are verified together on R027 as `IMPLEMENTED_PENDING_MERGE`; they are not merged to `master`. SCHWAB-001's listener and certificate-lifecycle preparation are `IMPLEMENTED_PENDING_MERGE` on its stacked successor branch. Phase 11's A017 paper API path remains `BLOCKED_VENDOR_CAPABILITY`. |
| Git sequencing | R012 and `origin/master` remain at `69feedf`; local `master` remains `164e32e`. R027 preserves the authorized two-parent integration of master plus R026. `codex/ARGUS-SCHWAB-001-loopback-oauth-listener` is the one allowed stacked successor. None of these branches may be rebased merely for cosmetic linearity. R026, TEST-001, R027, SCHWAB-001, Shadow-002, and Shadow-003 are unpushed. |
| Merge-base evidence | Local `master` and R026 resolve to `69feedf`. R027 begins at current master `164e32e` and integrates R026 `838ed22`; neither source branch is rewritten. |
...
| Immediate next action | SCHWAB-001 code requires no Steven Git-mechanics check. The next Schwab action is a security-state verification session: stage one production-local certificate version, confirm it is initially untrusted, install only its exact root into `CurrentUser\Root`, and prove `https://127.0.0.1:8182/oauth/callback` opens without a certificate warning before any credential or OAuth work. R027's four remaining visual surfaces may still be accepted or rejected when Steven next reviews the workstation. Do not attempt test-trade selection or populated-filter checks while the canonical count remains zero. Keep official sample collection locked throughout. |
| Remote backup action | Remote `master` remains at `69feedf`; local `master` remains ten commits ahead. R027, SCHWAB-001, and their source branches are not pushed. Do not push or merge them into `master` without Steven's separate explicit approval. |
| Broker and execution state | FakeBroker remains the only automated execution boundary. Schwab `Trader API - Individual` access is approved, and production app `Market Intelligence Workstation` is `Ready For Use` with `Accounts and Trading Production`, `Market Data Production`, callback `https://127.0.0.1:8182/oauth/callback`, and an order-request throttle of `5` per account per minute. SCHWAB-001 adds a credential-free, provider-free, one-use HTTPS receiver plus local root/leaf generation, AES-encrypted PKCS8 leaf keys, DPAPI-protected key-password storage, exact SAN/chain/ACL proof, and explicit install/remove trust confirmations. The Client ID and Client Secret exist in Schwab but have not been revealed, copied, or stored locally. No production certificate has been staged or trusted; no OAuth token, account hash, authenticated request, production endpoint client, paper/live broker path, or transmitting method exists. The registered products preserve future order capability, but Paper and Live remain software-locked. |
...
Status: `ACTIVE`; ARGUS-SHADOW-001, ARGUS-SHADOW-002, and ARGUS-SHADOW-003 are `COMPLETE` on local `master`; the official sample is `NOT_STARTED`; Schwab A017 is `BLOCKED_VENDOR_CAPABILITY`; Schwab application registration is `COMPLETE`; loopback OAuth listener and certificate-lifecycle preparation are `IMPLEMENTED_PENDING_MERGE` on `codex/ARGUS-SCHWAB-001-loopback-oauth-listener`; production trust installation is `MANUAL_PENDING_SECURITY_STATE_CHANGE`
...
- The Windows certificate-lifecycle preparation generates a five-year local root and a one-year server leaf limited to SANs `127.0.0.1` and `localhost`; never persists the root private key; writes only an AES-256-encrypted PKCS8 leaf key; stores its random password in current-user DPAPI; verifies exact certificate hashes, validity, hostname, chain, TLS handshake, and current-user-only explicit ACLs; versions material; refuses untrusted material by default; and requires exact confirmation phrases for install/remove. A failed install path attempts exact-thumbprint rollback. Passwords travel to PowerShell only through stdin and are excluded from command arguments and exception text.
- Certificate preparation proof uses real temporary Windows certificates and the actual one-use listener but does not modify `CurrentUser\Root`. Compileall passes; focused Schwab setup/listener/certificate tests pass 31/31; full Python discovery passes 666/666. No dependency, package, database, scoring, readiness, alert, trade-planning, broker, order, UI, or generated-data path changed.
- The callback is still not ready for a real OAuth attempt. No production certificate material exists under the default Local AppData path, no Momentum Hunter root is in `CurrentUser\Root`, and browser trust has not been physically verified. The listener is not wired to credential onboarding, an authorization URL, token exchange, token storage, account discovery, or WPF. Those are later checkpoints. The Schwab app remains modifiable if official runtime behavior requires a callback correction.
...
- Keep one active implementation branch and at most one stacked successor. R027 is the pending integration branch, and `codex/ARGUS-SCHWAB-001-loopback-oauth-listener` is its one active stacked successor under the standing credential-free Schwab authorization. No official sample collection may begin until Steven separately authorizes the exact sample definition.
...
Status: `ACTIVE`; R011/R012 are `COMPLETE`; R013-R025 plus Shadow-001/002/003 are `IMPLEMENTED_PENDING_MERGE` on R027
...
- R013 through R025 remain preserved on R026 and are fully integrated with the current Shadow baseline only on Steven-authorized R027.
...
- R027 passed Python compileall, 146 focused and 641 full-discovery Python tests, all 206 original .NET tests, zero-warning Release compilation, no-live-capability and protected-path review, source-nonmutation checks, and fresh offscreen UI proof. Manual review has passed the required `CRWV` / `5m` candle and hover cases and provisionally accepted the current Trade Plan evidence tabs pending broader market data. It found clipped interval text, meaningless sync labels, no-op pane actions, and unrecoverable pane removal. Repair commit `f84106a` passes a fresh Python compileall, all 641 Python tests, 162 focused presentation tests, all 209 .NET tests, and a zero-warning Release build. Live Windows verification confirms the truthful palette miss, complete Current pane menu, Research Maturity opening, single global mode treatment, first-class `Test Trade Review`, and final compact toolbar. Steven's visual acceptance remains pending.
- Steven approved a future integrated-workstation-chrome direction inspired by dense professional trading workstations: remove the separate light Windows title strip and make app identity, workspace navigation, the single global mode state, and window controls one continuous dark application surface. Implement this only as a separate post-R027 shell task. Preserve native drag, resize, Snap, double-click maximize/restore, minimize/maximize/close, `Alt+Space`, keyboard access, accessibility, high-DPI, and multi-monitor behavior. Keep one unmistakable global treatment for any future real-money mode rather than repeating warning labels across ordinary controls. This is visual shell work only and grants no broker, live-mode, credential, or execution authority.
...
- The future evidence ladder is: (1) FakeBroker prospective Shadow Trading; (2) manual thinkorswim paperMoney ticket/reconciliation; (3) Schwab contract emulator; (4) synthetic one-use HTTPS loopback callback and certificate-lifecycle proof, now `IMPLEMENTED_PENDING_MERGE` on SCHWAB-001; (5) production-local certificate staging, exact CurrentUser trust installation, and browser-warning-free proof, now `MANUAL_PENDING_SECURITY_STATE_CHANGE`; (6) separately authorized credential onboarding and OAuth; (7) Schwab authenticated read-only integration; (8) exact single-canary-account isolation proof; (9) broker preview only if official documentation proves a nontransmitting endpoint; (10) a separately approved supervised live canary; (11) reconciliation, audit review, and token-revocation drill; and (12) repeated supervised canary cycles.
