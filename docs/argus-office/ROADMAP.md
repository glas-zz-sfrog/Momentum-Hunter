...
| Active branch | `codex/ARGUS-R036-staged-candle-preview-host-7cbc2cb` is the verified backend-only successor stacked on `codex/ARGUS-R035-candle-input-reconcile-1af5b31`. `codex/ARGUS-SHADOW-016-017-reconcile-1af5b31` separately preserves verified post-opening evidence work; none of these branches moves the frozen scheduled checkout before the operational audit. |
...
| Immediate operational work | Preserve the final-head static bundle and exact installed-task definition, then inspect the one-time 2026-07-29 opening after its finite runner completes. Do not launch it early, retry a terminal failure, reuse expired proof, synthesize/backfill Trade 1, or bypass fresh quote/clock validation. Keep the verified R035, R036, and SHADOW-017 feature branches backed up; integrate them in ancestry order only after the frozen operational audit releases `master`. |
...
Status: `ACTIVE`; R011-R029 plus Shadow-001/002/003 are `COMPLETE` on local `master`; R035 and its backend-only R036 successor are `IMPLEMENTED_PENDING_MERGE`; remaining Qt retirement stays incremental
