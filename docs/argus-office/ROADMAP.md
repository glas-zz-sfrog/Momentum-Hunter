...
The next prospective namespace is `official-shadow-v2`. Its state, activation, policy, arm, and decision-cycle paths are distinct from v1, and regression proof shows v2 activation leaves every v1 byte unchanged. After SHADOW-017 is integrated and backed up, v2 is activated at `0 / 30`, a new final-head static bundle is prepared, and one armed FakeBroker-only task is installed for 2026-07-30 at 8:35 AM CT. The current stale host is intentionally not replaced during the 2026-07-29 entry window; tomorrow's scheduled preflight performs the guarded replacement before v2 can arm.
...
| Item | Current truth |
| --- | --- |
| Canonical baseline | After clean integration, `master` contains the WPF workstation through R029, Python engine contracts, Shadow-001 through Shadow-017, and SCHWAB-001/002/002A/003 read-only safeguards. The failed v1 opening evidence remains bound to `1af5b31`; v2 receives a new bundle prepared only from the final synchronized SHADOW-017 head. |
| Active branch | `codex/ARGUS-SHADOW-017-opening-runtime-preflight-repair` until verified fast-forward integration; no stacked implementation branch is active. |
...
| Scheduled operational proof | `SHADOW-017_VERIFIED_PENDING_FINAL_INSTALL`; after integration and backup, install one armed FakeBroker-only task for 2026-07-30 8:35 AM CT with no recurrence, late start, or Task Scheduler retry. The runner retains only its finite one-plus-three recognized-infrastructure attempts. |
| Immediate operational work | Integrate and back up SHADOW-017, activate isolated v2, prepare its final-head static bundle, install and independently inspect the one-time 2026-07-30 task, and attach one finite read-only observer. Do not launch early, reconstruct v1 Trade 1, or replace the stale production host during today's entry window. Independent work may continue outside the frozen task worktree. |
...
Status: `ACTIVE`; SHADOW-001 through SHADOW-016 and the Schwab read-only foundations are `COMPLETE` on `master`; SHADOW-017 is verified pending integration and final task installation; v1 is preserved as a failed prospective ceremony at `0 / 30`, v2 is the next isolated prospective sample, A017 is `BLOCKED_VENDOR_CAPABILITY`, and every real-order gate remains closed
