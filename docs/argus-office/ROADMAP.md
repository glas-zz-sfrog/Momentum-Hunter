...
Last reconciled: 2026-07-26 on `codex/ARGUS-SHADOW-006-deterministic-market-validity`. Local and remote `master` remain synchronized at `badee5c`. SHADOW-006, quote-provenance hardening, the read-only Schwab quote boundary, proof-backed selector arming, and its supported operational CLI are implemented and verified on the task branch, but the stack is not merged or armed. The branch replaces the temporary compile-time selector switch with an immutable write-once arm record tied to structured prerequisite proof artifacts, their hash-verified evidence, the exact activation record, the constitution hash, and a Shadow runtime build hash that includes the Schwab quote module. Runtime revalidates every persisted proof and evidence artifact; deletion, alteration, relocation, context mismatch, or build/constitution/activation drift fails closed. `selector-arm-check` performs the same complete verification without persisting policy, arm, cycle, state, or trade data. The separate `selector-arm` command requires the exact internal phrase and the same complete proof bundle; it creates no trade and exposes no broker call. The branch also implements canonical rank/score/identity ordering, fatal versus informational warnings, 30-second quote and multi-clock freshness gates, report/provider/quote identity, Opportunity ID and same-day duplicate controls, one global active position, regular-session/early-close/no-overnight behavior, expected-cycle and restart-downtime accounting, SPY/IWM and eligible-candidate counterfactuals, and the 10-session strategy-review gate. Active Monitor wrapper time cannot qualify copied bid/ask values as fresh. Production defaults to one exact-host Schwab Market Data quote GET, derives executable time from the oldest provider bid/ask/quote timestamp, refreshes OAuth only through the immutable sole-`2573`-CASH guard, rejects malformed, delayed, closed, extended-hours, stale, mismatched, non-finite, and unrequested evidence, and exposes no order method. A live Sunday read proved the real Schwab response shape and correctly rejected Friday CRWV/SPY quotes as stale, closed, extended-hours evidence. Compileall, 94 focused proof/selection/readiness/trading tests, full Python discovery at 841/841, and all 216 .NET tests pass. Production-local proof keeps activation SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`, arm/policy/cycle/state absent, and the official sample at `0 / 30`. FakeBroker remains the only automated execution boundary; Schwab remains read-only and bound to the sole `2573` `CASH` account.
...
| Active branch | `codex/ARGUS-SHADOW-006-deterministic-market-validity`, stacked on the remotely backed SHADOW-005 tip and backed up through the verified proof-backed selector arm and guarded operational CLI. SHADOW-006 is `IMPLEMENTED_PENDING_MERGE`; SHADOW-004 remains the visual parent pending its exact truth-label acceptance. |
...
| Immediate Builder work | After Steven is available for the one pending SHADOW-004 visual truth check, integrate the SHADOW-004/005/006 stack, refresh the workstation/Engine Host, and run the regular-market operational proof. Assemble the actual immutable proof bundle and create the write-once arm only after every prerequisite passes. Keep the official sample at zero until then. |
