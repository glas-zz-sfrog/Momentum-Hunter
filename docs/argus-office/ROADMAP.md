...
The separate routine Evening Capture ran at 7:00 PM CT on 2026-07-28 and completed successfully on its only attempt. Task Scheduler and the runner both report exit `0`; the job saved a ten-candidate `evening` capture and its three TradePlan reports, classified the market regime as `neutral`, and refreshed 1,121 outcome rows. This ordinary capture did not cross into protected Shadow collection state: the Shadow directory still contains only the immutable activation file with SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`; selector arm, selection policy, decision cycles, Shadow trade state, capture handoff, and Trade 1 remain absent.
...
SHADOW-016 closes the scheduler-shape gap before the first armed FakeBroker-only opening. An armed task is now rejected unless it is explicitly Shadow-only, enabled, future-dated, one-time, and scheduled at exactly 8:35 AM local Central time. The one-time task cannot start late when missed, receives zero Task Scheduler retries, and retains only the existing runner-owned initial attempt plus three finite infrastructure retries. A nonmutating plan mode proves the exact task action before registration; the default installer still plans three daily unarmed tasks and leaves Shadow disabled. PowerShell parsing, 3 focused scheduling tests, 130 affected Shadow/Engine Host tests, all 917 Python tests, and all 216 .NET tests pass. The installed 2026-07-29 task remains `Ready`, has one action and one trigger, `StartWhenAvailable` false, zero Task Scheduler retries, and an exact action containing one `-ArmShadowSelector`. It references `official-shadow-v1-selector-proof-bundle-1af5b31`; the bound task definition still has SHA-256 `6C3921D9FEAA946B4CEF2AD1A59088E87F4A46520E55241CCAB0E298FA9C386D`.

One finite read-only observer remains active for 8:50 AM CT. It inspects the completed Shadow task, final proof, protected state, Engine Host, redacted Schwab invariant, clock chronology, and the immediately preceding Evening Capture without launching, retrying, repairing, arming, selecting, or fabricating evidence. It retires only after a `COMPLETED` final audit; a terminal failure preserves evidence and does not retry the ceremony.
...
The post-freeze integration graph is now explicitly audited. R035, R036, and R037 form one linear chain. SHADOW-017 is a parallel sibling from `1af5b31`; neither it nor R036 contains the other. A read-only merge-tree check finds no overlapping runtime or test path and only four expected governance conflicts: `CHANGELOG_ARGUS.md`, `ROADMAP.md`, `TASK_LOG.md`, and `VERIFICATION_QUEUE.md`. A disposable detached rehearsal then applied all six SHADOW-017 runtime/test commits onto exact R036 without conflict. Python compileall, 82 focused integration tests, all 1,025 Python tests, all 216 .NET tests, and the warnings-as-errors Release build passed; the final diff contained only the nine expected Shadow evidence/runtime test files and no secret or unrelated protected-path hit. A second detached rehearsal applied the exact R037 source/test patch on top of that combined state. Stable patch IDs matched both source branches, Python compileall passed, all 224 .NET tests passed, the warnings-as-errors Release build passed, and no secret value appeared. Both rehearsal worktrees were removed and created no branch or commit. This proves mechanical compatibility only; Steven's visual acceptance remains required. After the operational audit releases `master`, fast-forward R035 and R036 first. Then replay SHADOW-017's six focused commits onto that released baseline on a new reconciliation branch, regenerate the four governance files from observed truth, rerun the combined proof, and fast-forward only the verified result. If SHADOW-017 lands before R037 is visually accepted, replay R037 implementation commit `c5f287a` onto the then-current baseline after acceptance rather than directly merging the old sibling branch. Do not rebase, reset, or create a non-fast-forward merge.

SHADOW-017 is `IMPLEMENTED_PENDING_MERGE` on `codex/ARGUS-SHADOW-016-017-reconcile-1af5b31`. It reconciles the useful code from an older stacked branch onto the canonical SHADOW-016 scheduler baseline without replaying stale governance or moving the proof-bound `master`. The evidence layer records manual thinkorswim paperMoney results as separate write-once artifacts, quantifies entry/exit/quantity/P&L differences against frozen FakeBroker evidence, audits model error without combining fill-model versions or releasing sub-30 statistics, and automatically freezes immutable 5/10/20/30 official-sample checkpoints. Automatic checkpoint code is included in the selector runtime hash; an unresolved exact-threshold checkpoint blocks the next quote, and a post-transition write failure is recovered before the next quote is processed. Compileall, 46 focused tests, 187 Shadow tests, 35 Engine Host/client tests, all 962 Python tests, and all 216 .NET tests pass. No generated report is tracked, and no account, credential, network, broker, order, scoring, readiness, database/schema, or UI path changed.
...
| Active branch | `codex/ARGUS-INTEGRATION-003-preopening-rehearsal` combines the verified R035-R037 candle/UI stack with SHADOW-017 onward only for isolated compatibility proof. The source branches remain backed up, and none of this work moves the frozen scheduled checkout before the operational audit. |
...
| Scheduled operational proof | `SCHEDULED_PENDING_RUN`; the prior 2026-07-28 proof-only task passed, SHADOW-015 passed `3 / 3`, tonight's routine Evening Capture exited `0` without creating Shadow state, and the replacement task is one-time at 2026-07-29 8:35 AM CT with an explicit arm switch and no scheduler recurrence or restart. The finite read-only observer remains scheduled for 8:50 AM CT. |
| Immediate operational work | Preserve the final-head static bundle and exact installed-task definition, then inspect the one-time 2026-07-29 opening after its finite runner completes. Do not launch it early, retry a terminal failure, reuse expired proof, synthesize/backfill Trade 1, or bypass fresh quote/clock validation. Continue isolated implementation and integration rehearsal meanwhile. After the audit releases `master`, replay the proven combined stack onto that released baseline through the normal Git proof gates; R037 still requires Steven's exact visual acceptance before final integration. |
...
Status: `ACTIVE`; SHADOW-001 through SHADOW-016 and the Schwab read-only foundations are `COMPLETE` on `master`; SHADOW-017 is `IMPLEMENTED_PENDING_MERGE`; the official sample is `ACTIVATED`, `SELECTOR_NOT_ARMED`, `SCHEDULED_PENDING_RUN`, and `0 / 30`; A017 is `BLOCKED_VENDOR_CAPABILITY`; every real-order gate remains closed
...
- SHADOW-017 implements the evidence follow-through without changing the sample. Manual thinkorswim paperMoney entries are write-once records bound to frozen Shadow trade/order/plan/risk/selector/cycle identity. A separate read-only audit reports paper-minus-FakeBroker entry, exit, quantity, total-P&L, and per-share deltas, keeps fill-model versions separate, and withholds descriptive aggregates below 30 comparable observations.
- SHADOW-017 also implements hash-addressed, write-once evidence checkpoints at 5, 10, 20, and 30 completed eligible trades. Exact 5/10/20 checkpoints retain canonical gated metrics, exact 30 may expose descriptive canonical metrics only, and late reconstruction withholds aggregate metrics. Both observation paths invoke checkpointing after finalized state, while exact-threshold preflight and runtime-hash binding prevent silent advancement past unresolved evidence. Interim reports evaluate mechanics and evidence quality and must not tune the strategy to the developing sample.
