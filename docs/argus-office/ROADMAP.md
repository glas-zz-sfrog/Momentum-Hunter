...
| Active branch | `codex/ARGUS-SHADOW-017-evidence-checkpoints` at `168c52f`, stacked on verified SHADOW-016 tip `a2c3637`; immutable 5/10/20/30 checkpoint generation is implemented and verified pending governance closeout and branch backup. |
...
- SHADOW-017 implements immutable evidence checkpoints at 5, 10, 20, and 30 completed eligible trades on its stacked feature branch. It consumes only the canonical read-only Shadow review projection, sorts by the actual offset-aware decision instant, validates sample/activation/evidence identity, freezes the first `N` eligible PASS records with a hash-addressed manifest, and writes an idempotent JSON/Markdown pair outside source-state storage. Exact 5/10/20 snapshots retain canonical gated metrics; an exact 30 snapshot may expose descriptive canonical metrics only; any threshold reconstructed after the count has advanced withholds aggregate metrics. Tampered, conflicting, duplicated, count-mismatched, concurrently changing, transmitting, or unauthorized evidence fails closed. The engine never mutates Shadow state or activation, calls a broker or network, creates an order, tunes strategy, or authorizes trading. Compileall, 13 focused tests, all 176 Shadow tests, all 948 Python tests, and all 216 .NET tests pass at `168c52f`; no generated checkpoint report is tracked.
