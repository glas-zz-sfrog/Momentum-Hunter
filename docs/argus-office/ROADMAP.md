...
| Active branch | `codex/ARGUS-SCHWAB-004-account-shape-evidence` through SCHWAB-004A implementation `3b65e16` plus governance closeout. It contains the confirmation-gated, GET-only, value-erasing live account-shape inspector and a separate offline-only account OpenAPI inspector on top of the backed-up ARGUS-INTEGRATION-002 release gate. The offline inspector has no provider, credential, persistence, position, order, or execution surface and never turns structural field evidence into funding authority. Compileall, 17 focused, 405 Schwab, all 1,252 Python, and all 216 .NET tests pass. |
...
| Immediate operational work | Allow the one-time proof-only task and read-only heartbeat to run without manual launch, retry, repair, arm, selection, or backfill. Continue independent offline contract/guard/evidence work on the sole stacked successor. Use SCHWAB-004A on an authenticated official local account-schema export when one is available, then manually review official descriptions without guessing from live values. After the audit releases the proof-bound baseline, run `python -B tools/verify_pending_foundation.py`, regenerate the CANARY manifest for the exact final stack, and rerun the complete suites. Fast-forward `master` only when all gates pass and ancestry remains exact; if the audit advances `master`, rebuild the linear successor from that new head instead of rebasing or forcing. |
...
- CANARY-001/002/003/004/005/006/007/008/009/010/011/012/013/014 and SCHWAB-004/004A are prerequisites, not canary authorization. Current runtime evidence still blocks because settled-cash and complete restriction semantics, authenticated official Schwab account/order OpenAPI exports and resulting reviewed mappings, binding the persisted process-evidence path to the exact future broker-worker identity, provider credential-revocation evidence, and a physical full stop drill are unavailable. Steven's eventual physically verified exact decision is also still absent. The live value-free shape is provider evidence about field presence/types only; it does not prove financial semantics, funding sufficiency, or order behavior. The synthetic emulator, integration manifest, offline schema inspectors, and isolated child-process persistence proof likewise are not live-order or runtime-stop proof. No pending branch can preview, submit, replace, cancel, or transmit an order. Remaining gates include authoritative field semantics, inspection of the official exports, implementation and proof of any supported GET-only order mapping, branch integration after the proof-bound baseline is released, broker-worker identity binding, provider revocation wiring and credential remediation, operational proof, and Steven's explicit decision for an actual order.
