...
| Item | Current truth |
| --- | --- |
| Canonical baseline | `master` contains the WPF workstation through R029, Python engine contracts, Shadow-001 through Shadow-010, and SCHWAB-001/002/002A/003 read-only safeguards. SHADOW-010 automates the proof-complete arm ceremony immediately before the existing 9:35 AM ET opening selector handoff. |
| Active branch | No implementation branch is active. `codex/ARGUS-SHADOW-010-automatic-proof-ceremony` and earlier Shadow branches are preserved as merged source history. |
