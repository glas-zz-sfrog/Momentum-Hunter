...
Last reconciled: 2026-07-26 on `master`. The complete SHADOW-004/005/006 stack is fast-forward integrated through visual-acceptance commit `307a2e1` and is the canonical implementation. Steven accepted all six live SHADOW-004 truth-label and layout checks. The canonical runtime replaces the temporary compile-time selector switch with an immutable write-once arm record tied to structured prerequisite proof artifacts, their hash-verified evidence, the exact activation record, the constitution hash, and a Shadow runtime build hash that includes the Schwab quote module. Runtime revalidates every persisted proof and evidence artifact; deletion, alteration, relocation, context mismatch, or build/constitution/activation drift fails closed. `selector-arm-check` performs the same complete verification without persisting policy, arm, cycle, state, or trade data. The separate `selector-arm` command requires the exact internal phrase and the same complete proof bundle; it creates no trade and exposes no broker call. A dedicated `schwab_market_data proof` command makes the pre-arm regular-market gate executable without arming: it performs the same exact-host GET, enforces the frozen 30-second age and independent provider clocks, rejects future, stale, delayed, closed, extended, nontradable, missing, crossed, mismatched, and nonfinite evidence, writes only redacted derived proof, and has no account or order endpoint. Canonical selection now includes rank/score/identity ordering, warning severity, report/provider/quote identity, Opportunity ID and same-day duplicate controls, one global active position, session/early-close/no-overnight behavior, expected-cycle and restart-downtime accounting, SPY/IWM and eligible-candidate counterfactuals, and the 10-session strategy-review gate. A live Sunday read proved the real Schwab response shape and correctly rejected Friday CRWV/SPY quotes as stale, closed, extended-hours evidence; it does not satisfy the pending open-market proof. Compileall, 110 focused Schwab/proof/selection/readiness/trading tests, full Python discovery at 844/844, all 216 .NET tests, a zero-warning Release build, and live visual proof pass. Production-local proof keeps activation SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`, arm/policy/cycle/state absent, and the official sample at `0 / 30`. FakeBroker remains the only automated execution boundary; Schwab remains read-only and bound to the sole `2573` `CASH` account.
...
| Blocked by | One regular-market operational proof that Schwab provides a tradable quote inside the frozen 30-second boundary, followed by a complete non-synthetic proof bundle whose structured attestations and referenced evidence pass v2 arm verification. |
| Immediate Builder work | During the next regular market, run the read-only proof command for the current candidate plus SPY/IWM and require an actual `PASS`. Assemble the immutable proof bundle and create the write-once arm only after every prerequisite passes. Keep the official sample at zero until then. |
...
Status: `ACTIVE`; SHADOW-001 through SHADOW-006 and the Schwab read-only foundations are `COMPLETE` on `master`; the official sample is `ACTIVATED` with `SELECTOR_NOT_ARMED` and `0 / 30` completed trades; the regular-market quote proof is pending; A017 is `BLOCKED_VENDOR_CAPABILITY`; every real-order gate remains closed
