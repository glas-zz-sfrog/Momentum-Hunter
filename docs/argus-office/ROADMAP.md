...
Last reconciled: 2026-07-27 on `master`. The complete SHADOW-004 through SHADOW-009 stack is fast-forward integrated and backed up. SHADOW-009 closes two final pre-arm integrity gaps. First, live quote proof must derive the highest canonical-ranked candidate from the newest fresh scheduled report and preserve byte-exact copies plus hashes of that report and its immutable source capture; a caller-picked ticker can no longer satisfy Gate 8. Second, Official Shadow V1 now has one distinct immutable `shadow` capture per XNYS market-open day at 9:35 AM ET, followed immediately by the existing authenticated loopback Engine Host collection/selector cycle. The 7:00 AM premarket capture remains separate. A five-minute launch window, market-calendar check, report/capture freshness matrix, deterministic report-hash command ID, write-once handoff receipt, missing-receipt retry, host non-overlap guard, selector idempotency, and no-rescan duplicate handling make the path fail closed and at-least-once without permitting duplicate official trades. The arm runtime hash and `cycle_accounting` proof now include this cadence and handoff path. Compileall, 193 affected tests, all 37 named proof-gate tests, full Python discovery at 871/871, and all 216 .NET tests pass. Production-local proof keeps activation SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`, arm/policy/cycle/state/trade/handoff absent, and the official sample at `0 / 30`. FakeBroker remains the only automated execution boundary; Schwab remains read-only and bound to the sole `2573` `CASH` account.

Backup state: local and remote `master` contain the integrated SHADOW-009 implementation. The prior ignored SHADOW-008 static bundle is preserved but intentionally stale because canonical Git and the expanded runtime hash changed; a uniquely named SHADOW-009 bundle must be prepared only after final synchronization.
...
| Item | Current truth |
| --- | --- |
| Canonical baseline | `master` contains the WPF workstation through R029, Python engine contracts, Shadow-001 through Shadow-009, and SCHWAB-001/002/002A/003 read-only safeguards. SHADOW-009 binds live proof to the canonical report/capture and supplies the 9:35 AM ET opening capture-to-selector cadence. |
| Active branch | No implementation branch is active. `codex/ARGUS-SHADOW-009-live-proof-report-binding` and earlier Shadow branches are preserved as merged source history. |
...
| Blocked by | One regular-market operational proof that Schwab provides the report-derived candidate plus SPY/IWM as tradable quotes inside the frozen 30-second boundary. The SHADOW-009 static 11-of-12 bundle must be regenerated from synchronized canonical Git first. |
| Immediate Builder work | Prepare the uniquely named SHADOW-009 static bundle, verify the installed 9:35 AM ET Windows task, then use the first fresh opening report for the read-only candidate-plus-SPY/IWM proof. Finalize, run `selector-arm-check`, and create the write-once arm only if every verifier returns `PASS`; then run the immediate selector cycle within the frozen report-to-selection window. |
...
Status: `ACTIVE`; SHADOW-001 through SHADOW-009 and the Schwab read-only foundations are `COMPLETE` on `master`; the official sample is `ACTIVATED` with `SELECTOR_NOT_ARMED` and `0 / 30` completed trades; SHADOW-009 static regeneration and the regular-market quote proof are pending; A017 is `BLOCKED_VENDOR_CAPABILITY`; every real-order gate remains closed
