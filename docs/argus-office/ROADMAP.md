...
| Active branch | `codex/ARGUS-SCHWAB-004-account-shape-evidence` through CANARY-016 implementation `4d3c9db` plus governance closeout. It contains SCHWAB-004/004A account evidence, CANARY-015 identity binding, and CANARY-016's bounded local worker lifecycle on top of the backed-up ARGUS-INTEGRATION-002 release gate. The worker can emit immutable local identity/stop evidence and self-exit, but has no provider, credential, account-query, broker, order, retry, or execution surface. Compileall, 12 focused, 102 adjacent CANARY, 433 Schwab, all 1,280 Python, and all 216 .NET tests pass. |
...
| Immediate operational work | Allow the one-time proof-only task and read-only heartbeat to run without manual launch, retry, repair, arm, selection, or backfill. Continue independent nontransmitting contract/guard/evidence work on the sole stacked successor. Use SCHWAB-004A on an authenticated official local account-schema export when available. Preserve CANARY-015/016 as the identity and lifecycle boundary for a future provider-coupled worker, but do not mistake the local child-process proof for credential revocation, provider behavior, a physical full stop drill, or execution authority. After the audit releases the proof-bound baseline, run `python -B tools/verify_pending_foundation.py`, regenerate the CANARY manifest for the exact final stack including CANARY-015/016, and rerun the complete suites. Fast-forward `master` only when all gates pass and ancestry remains exact. |
...
- CANARY-001 through CANARY-016 and SCHWAB-004/004A are prerequisites, not canary authorization. CANARY-016 proves that the reviewed local worker shell can emit a real identity receipt, accept an independently persisted stop latch, acknowledge it, self-exit, and be independently observed stopped in a synthetic Windows child lifecycle. It does not access Schwab, credentials, account state, broker state, or orders, so it is not the eventual provider-coupled worker or a complete physical stop drill. Current runtime evidence still blocks because settled-cash and complete restriction semantics, authenticated official Schwab account/order OpenAPI exports and resulting reviewed mappings, provider-coupled worker identity/process evidence, provider credential-revocation evidence, credential remediation, and a physical full stop drill are unavailable. Steven's eventual physically verified exact decision is also still absent. The live value-free shape, synthetic emulator, integration manifest, offline schema inspectors, and isolated child-process proof do not establish live-order authority. No pending branch can preview, submit, replace, cancel, or transmit an order. Remaining gates include authoritative field semantics, inspection of the official exports, proof of any supported GET-only order mapping, branch integration after the proof-bound baseline is released, provider-coupled worker/revocation wiring, operational proof, and Steven's explicit decision for an actual order.
