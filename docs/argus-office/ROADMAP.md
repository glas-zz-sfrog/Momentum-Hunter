...
Status: `ACTIVE`; SHADOW-001 through SHADOW-014 preparation and the Schwab read-only foundations are `COMPLETE` on `master`; SHADOW-016, SHADOW-017, and CANARY-001 are `IMPLEMENTED_PENDING_MERGE` on independent feature history; the official sample is `ACTIVATED`, `SELECTOR_NOT_ARMED`, `OPENING_CEREMONY_UNPROVEN`, and `0 / 30`; A017 is `BLOCKED_VENDOR_CAPABILITY`; every real-order gate remains closed
