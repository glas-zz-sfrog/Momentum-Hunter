...
| Item | Current truth |
| --- | --- |
| Canonical baseline | After clean integration, `master` contains the WPF workstation through R029, Python engine contracts, Shadow-001 through Shadow-016, and SCHWAB-001/002/002A/003 read-only safeguards. The completed proof-only opening remains bound to `4c35181`; the next static bundle is prepared only from the final synchronized SHADOW-016 head. |
| Active branch | `codex/ARGUS-SHADOW-016-017-reconcile-1af5b31`; SHADOW-017 is verified and remains separate from `master` until the 2026-07-29 proof-bound opening audit releases the frozen `1af5b31` baseline. |
...
| Blocked by | No unresolved implementation or synthetic-proof gate. The first armed FakeBroker-only collection is operationally time-gated until 2026-07-29 at 8:35 AM CT and must fail closed on any stale proof, Git mismatch, brokerage anomaly, or protected-state change. Product development is not blocked. |
...
| Immediate operational work | Preserve the final-head static bundle and exact installed-task definition, then inspect the one-time 2026-07-29 opening after its finite runner completes. Do not launch it early, retry a terminal failure, reuse expired proof, synthesize/backfill Trade 1, or bypass fresh quote/clock validation. Keep the verified SHADOW-017 branch separate until that audit releases the baseline, then fast-forward it through the normal Git proof gates. |
