...
Last reconciled: 2026-07-27 on `codex/ARGUS-R035-candle-input-hardening`, based on clean synchronized `master` at `4c35181`. SHADOW-014 is integrated, backed up, and scheduled for one unarmed proof-only run on 2026-07-28 at 8:35 AM Central. Its earlier 8:50 observer correctly classified the disabled, never-run 8:35 task as `FAILED`: no opening log, final-head bundle, capture, report, quote/clock proof, arm, policy, decision cycle, handoff, or trade existed, and no retry was attempted. The sanitized 20-file audit bundle is preserved outside the repository under `ArgusReviewBundles`.
...
SHADOW-015 physically rehearsed the three non-market failure/in-progress cases on 2026-07-27. A real temporary Windows task and process classified `IN_PROGRESS` without retiring the heartbeat; a structured Engine Host rejection classified `FAILED` and could not create a terminal handoff; and a 7.025-second synthetic HTTPS clock skew plus 1.050-second uncertainty failed the five-second gate before arm, cycle, handoff, or trade. The temporary task/process were removed, the production task definition/state and sole activation file remained unchanged, and the sanitized six-file external bundle manifest is SHA-256 `996204108C83E522AF769EAF21E6A7F1B82C06B3D5DFEBEA7FCBB3E14CC1DE36`.
...
Backup state: local and remote `master` are synchronized at SHADOW-014 scheduling commit `4c35181`. The static bundle, exported one-time task definition, and inspection heartbeat bind to that exact canonical commit. Separately, R030-R035 are implemented and verified through R035 implementation commit `f4fc613`; SHADOW-015 evidence remains in the stacked history and externally under `ArgusReviewBundles`. The stack remains unmerged so the scheduled proof baseline cannot move.

R035 closes the bounded-input gap found during the second-pass R030-R034 security review. Schwab price-history bodies now stream through a four-megabyte decoded-byte ceiling, close the response on every completed-header path, and enforce interval-specific candle-count caps before parsing. Persisted target and staged-chart inputs reject oversized files before parsing, while large file and SQLite fingerprints are computed incrementally. Compileall, 52 affected tests, 960/960 full Python tests, and 216/216 .NET tests pass. A fresh read-only live preflight covered 22/22 targets and 44/44 interval results, reproduced 710 exact legacy rows and zero unrelated/alias/sidecar state, and preserved all three canonical source hashes. Its external sanitized proof manifest is SHA-256 `A58EDB7464D15683107975F70350AF8D2EC1C4804C4B5DA0CF44A0E8AF12E49C`; no raw candles, token, credential, or account identity are included.
...
| Item | Current truth |
| --- | --- |
| Canonical baseline | Local and remote `master` are clean and synchronized at `4c35181`; they contain the WPF workstation through R029, Python engine contracts, Shadow-001 through scheduled SHADOW-014, and SCHWAB-001/002/002A/003 read-only safeguards. |
| Active branch | `codex/ARGUS-R035-candle-input-hardening`, stacked on verified R030-R034 and SHADOW-015 through implementation commit `f4fc613`. The complete stack remains `IMPLEMENTED_PENDING_MERGE` and will be backed up without moving canonical `master`. |
...
| Blocked by | Phase 11 opening-proof acceptance now requires only the scheduled real market-day proof-only run proving candidate+SPY/IWM quote, clock, chronology, configuration, runner retry, and 12/12 finalized bundle behavior while arm/policy/cycle/handoff/trade remain absent. The structured-host, clock-failure, and running-at-heartbeat rehearsals pass. This gate does not freeze isolated Phase 12 development; R030-R034 integration waits only to avoid moving the proof-bound canonical commit. |
...
| Immediate operational work | Allow the one-time proof-only task and read-only heartbeat to run. Do not manually launch, retry, repair, arm, select, or synthesize/backfill Trade 1. After its audit, fast-forward the verified R030-R035 stack, run the write-once full-target cutover preflight from the integrated commit, then stop at the explicit destructive cutover decision. |
...
| Data caveat | Legacy/current persisted bid/ask rows with only monitor-cycle timestamps remain unavailable rather than presumed fresh. The read-only Schwab transport and clock path passed a regular-session SPY/IWM check, but the candidate-bound opening proof remains pending. R030-R034 proved fresh inactive Schwab candle staging, complete 22-target coverage, and exact cutover inventory in one write-once preflight without changing the active source. Only legacy `CRWV` is currently active; actual-data cutover remains a destructive-operation interruption gate. The frozen early-close table covers 2026-2028 and fails closed beyond it. |
...
- SHADOW-015 failure/in-progress physical evidence is `AUTOMATED_PASS` on the stacked successor branch. The real temporary task/process produced `IN_PROGRESS`; structured Engine Host rejection and greater-than-five-second clock skew produced `FAILED`; neither retired the heartbeat or created arm/policy/cycle/handoff/trade state. The sanitized evidence bundle is `C:\Users\steve\OneDrive\Documents\ArgusReviewBundles\SHADOW-015-opening-failure-rehearsals-20260727T183545Z`, manifest SHA-256 `996204108C83E522AF769EAF21E6A7F1B82C06B3D5DFEBEA7FCBB3E14CC1DE36`.
...
- R030-R035 are `IMPLEMENTED_PENDING_MERGE` on `codex/ARGUS-R035-candle-input-hardening` through implementation commit `f4fc613`, intentionally absent from canonical `master` until the scheduled SHADOW-014 proof is audited. R030 adds an exact-host GET-only Schwab price-history source for 1-minute and Daily bars; R031 binds inactive staging to the newest persisted monitor-target report; R032 exposes only a validated read-only preview in the existing chart snapshot shape; R033 inventories the exact legacy artifacts, SQLite rows, aliases, sidecars, and full staged coverage without deleting, updating, inserting, or activating anything.
- The live R033 proof covered all 22 persisted targets and 44 symbol/interval results, found exactly 710 legacy `CRWV` SQLite rows, zero unrelated rows, zero path/hash aliases, and zero database sidecars, and classified the state `READY_FOR_EXPLICIT_DESTRUCTIVE_DECISION`. That classification grants no permission: `deletionPerformed`, `databaseMutationPerformed`, `activeChartSourceChanged`, and `cutoverPermitted` all remain false.
- R034 composes full-target selection, fresh 1-minute/Daily staging, hash-verified preview identity, exact legacy/SQLite inventory, source-nonmutation comparison, and one required write-once receipt into a single fail-closed command. It refuses truncated target sets, protected or reused receipt paths, provider/clock/identity failure, partial evidence, source drift, SQLite aliases, unexpected rows, and sidecars; it has no activation, deletion, database-write, account-data, order, or transmission path.
- The live R034 preflight covered 22/22 targets and 44/44 available interval results, found the same 710 legacy rows with zero unrelated rows, aliases, or sidecars, and left every canonical input hash unchanged. Compileall, 47 focused/adjacent tests, 955/955 full Python tests, 216/216 .NET tests, diff/secret/destructive-path review, and source-nonmutation proof pass.
- R035 hardens the same read-only path against oversized or streaming inputs. Provider responses are bounded while streaming, interval candle collections are capped before object parsing, local target/stage files are size-checked before bounded reads, and file/SQLite hashes are incremental. Redirects remain disabled, every completed-header response is closed, and stream failures are redacted. Compileall, 52 affected tests, 960/960 full Python tests, 216/216 .NET tests, diff/secret/protected-path review, and a fresh 22-target/44-result live preflight pass with canonical source hashes unchanged.
- Next Phase 12 action: after SHADOW-014 audit, integrate R030-R035 by clean fast-forward, rerun the required write-once preflight from the integrated commit, and present Steven the exact two legacy files plus 710 SQLite rows immediately before any destructive cutover. Do not implement or perform the cutover before that interruption.
