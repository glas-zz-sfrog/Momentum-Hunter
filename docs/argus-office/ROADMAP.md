...
Last reconciled: 2026-07-26 on `codex/ARGUS-SHADOW-006-deterministic-market-validity`. Local and remote `master` remain synchronized at `badee5c`. SHADOW-006, quote-provenance hardening, the read-only Schwab quote boundary, proof-backed selector arming, and their supported operational CLIs are implemented and verified on the task branch, but the stack is not merged or armed. The branch replaces the temporary compile-time selector switch with an immutable write-once arm record tied to structured prerequisite proof artifacts, their hash-verified evidence, the exact activation record, the constitution hash, and a Shadow runtime build hash that includes the Schwab quote module. Runtime revalidates every persisted proof and evidence artifact; deletion, alteration, relocation, context mismatch, or build/constitution/activation drift fails closed. `selector-arm-check` performs the same complete verification without persisting policy, arm, cycle, state, or trade data. The separate `selector-arm` command requires the exact internal phrase and the same complete proof bundle; it creates no trade and exposes no broker call. A dedicated `schwab_market_data proof` command now makes the pre-arm regular-market gate executable without arming: it performs the same exact-host GET, enforces the frozen 30-second age and independent provider clocks, rejects future, stale, delayed, closed, extended, nontradable, missing, crossed, mismatched, and nonfinite evidence, writes only redacted derived proof, and has no account or order endpoint. The branch also implements canonical rank/score/identity ordering, warning severity, report/provider/quote identity, Opportunity ID and same-day duplicate controls, one global active position, session/early-close/no-overnight behavior, expected-cycle and restart-downtime accounting, SPY/IWM and eligible-candidate counterfactuals, and the 10-session strategy-review gate. A live Sunday read proved the real Schwab response shape and correctly rejected Friday CRWV/SPY quotes as stale, closed, extended-hours evidence; it does not satisfy the pending open-market proof. Compileall, 110 focused Schwab/proof/selection/readiness/trading tests, full Python discovery at 844/844, and all 216 .NET tests pass. Production-local proof keeps activation SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`, arm/policy/cycle/state absent, and the official sample at `0 / 30`. FakeBroker remains the only automated execution boundary; Schwab remains read-only and bound to the sole `2573` `CASH` account.
...
| Active branch | `codex/ARGUS-SHADOW-006-deterministic-market-validity`, stacked on the remotely backed SHADOW-005 tip and backed up through the verified proof-backed selector arm, guarded arm CLI, and pre-arm regular-market proof CLI. SHADOW-006 is `IMPLEMENTED_PENDING_MERGE`; SHADOW-004 remains the visual parent pending its exact truth-label acceptance. |
...
| Blocked by | Canonical integration and backup of SHADOW-004/005/006; pending SHADOW-004 WPF truth-label acceptance; a refreshed canonical workstation/Engine Host after integration; one regular-market operational proof that Schwab provides a tradable quote inside the frozen 30-second boundary; and a complete non-synthetic proof bundle whose structured attestations and referenced evidence pass v2 arm verification. |
| Immediate Builder work | After Steven is available for the one pending SHADOW-004 visual truth check, integrate the SHADOW-004/005/006 stack and refresh the workstation/Engine Host. During the next regular market, run the read-only proof command for the current candidate plus SPY/IWM and require an actual `PASS`. Assemble the immutable proof bundle and create the write-once arm only after every prerequisite passes. Keep the official sample at zero until then. |
