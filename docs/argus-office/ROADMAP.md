...
| Immediate next action | Preserve synchronized `master` and resolve the next explicit product gate before Builder work: real Schwab credential onboarding/OAuth and official Shadow sample start each require their own authorization. Do not infer either authorization from the completed backup. |
