...
On stacked branch
`codex/ARGUS-STAT-DATA-001-prospective-opportunity-denominator`, the general
prospective opportunity denominator is
`IMPLEMENTED_PENDING_PARENT_INTEGRATION` from Specialist Contract parent
`e65cb70`. It freezes separate immutable cycle and opportunity records,
explicit complete/incomplete denominator proof, provider-bound and failure
rows, common-contract specialist attachments, and separate market-path,
broker-execution, and data-quality outcomes. Persistence is atomic,
write-once, caller-root-only, and unconsumed by current runtime code. Sample
`opportunity-denominator-research-v1` remains inactive at zero sessions and
zero opportunities. A read-only in-memory adaptation of the August 14 opening
report preserved SNDK and NU but correctly classified the source as
`RETROSPECTIVE_RESEARCH_EXAMPLE / DENOMINATOR_INCOMPLETE` because the report
represented only 2 of 20 parsed source rows. Nothing was persisted, activated,
scheduled, installed, merged, or repinned.
