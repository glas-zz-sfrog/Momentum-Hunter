...
Last reconciled: 2026-07-27 on `master`. The complete SHADOW-004 through SHADOW-012 stack is fast-forward integrated and backed up. SHADOW-009 binds live quote proof to the highest canonical-ranked candidate from the newest fresh scheduled report and gives Official Shadow V1 one distinct immutable `shadow` capture per XNYS market-open day at 9:35 AM ET. SHADOW-010 removes the remaining need for Steven to be present at the opening gate: after the capture/report exists, the scheduled job preflights synchronized canonical Git and all 11 static artifacts, validates the report/capture identity and freshness, requests only the report-derived candidate plus SPY/IWM from the existing read-only Schwab quote endpoint, retries a failed quote proof three times, finalizes the twelfth artifact, reruns the existing complete prerequisite verifier, creates the write-once selector arm only after every gate passes, and then invokes the existing authenticated loopback Engine Host cycle. A pre-open SHADOW-011 audit caught and fixed a timestamp-ordering defect before the first live run: proof evaluation now takes one clock reading before OAuth/provider work and a second after the response, so a valid quote observed during request latency is measured against the post-response clock rather than misclassified as future data. SHADOW-012 closes the corresponding scheduler retry gap: the opening task alone receives three one-minute Windows restarts, allowing the existing write-once/report-hash recovery path to resume a transiently failed ceremony inside the frozen five-minute window. `IgnoreNew` prevents overlap, and the morning/evening task behavior is unchanged. Compileall, 237 affected tests, all 46 named proof-gate tests, full Python discovery at 882/882 through SHADOW-011, all 216 .NET tests, PowerShell parsing, and direct scheduled-task settings construction pass. Production-local proof keeps activation SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`, arm/policy/cycle/state/trade/handoff absent, and the official sample at `0 / 30`. FakeBroker remains the only automated execution boundary; Schwab remains read-only and bound to the sole `2573` `CASH` account.

Backup state: local and remote `master` contain the integrated SHADOW-012 implementation. All earlier ignored static bundles remain preserved but intentionally stale because canonical Git and the expanded runtime hash changed. Only the uniquely named bundle prepared from the final synchronized SHADOW-012 `master` HEAD may be installed for the opening task.
...
| Item | Current truth |
| --- | --- |
| Canonical baseline | `master` contains the WPF workstation through R029, Python engine contracts, Shadow-001 through Shadow-012, and SCHWAB-001/002/002A/003 read-only safeguards. SHADOW-012 gives the opening ceremony bounded one-minute task retries while preserving SHADOW-011's post-response clock. |
| Active branch | No implementation branch is active. `codex/ARGUS-SHADOW-012-scheduler-retry` and earlier Shadow branches are preserved as merged source history. |
...
| Active decision | Use deterministic automatic selection, not operator-selected official trades. Run the opening proof/arm ceremony automatically after the fresh 9:35 AM ET report, but fail closed unless every frozen prerequisite independently passes. Thirty trades is an engineering gate, not proof of edge or live authorization. |
| Blocked by | One live market-day run must prove that Schwab provides the report-derived candidate plus SPY/IWM as tradable regular-session quotes inside the frozen 30-second boundary. Until then the selector remains unarmed and the sample remains `0 / 30`. |
| Immediate operational work | Prepare the final-HEAD 11-of-12 static bundle, install and inspect the exact 8:35 AM CT task action, keep the canonical Engine Host current, and let the first live market-day task run the guarded proof/arm/handoff sequence. Inspect the immutable result and task log afterward; do not synthesize or backfill Trade 1. |
...
Status: `ACTIVE`; SHADOW-001 through SHADOW-012 and the Schwab read-only foundations are `COMPLETE` on `master`; the official sample is `ACTIVATED` with `SELECTOR_NOT_ARMED` and `0 / 30` completed trades; the first automatic regular-market proof/arm/handoff run is pending; A017 is `BLOCKED_VENDOR_CAPABILITY`; every real-order gate remains closed
...
- SHADOW-012 configures three one-minute Windows restarts for the Shadow opening task only. Retries remain inside the scheduling policy's five-minute window, cannot overlap, reuse the immutable report and deterministic report-hash command, and stop once the write-once handoff receipt exists.
