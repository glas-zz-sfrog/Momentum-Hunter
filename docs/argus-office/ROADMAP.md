...
Last reconciled: 2026-07-26 on `codex/ARGUS-SHADOW-006-deterministic-market-validity`. Local and remote `master` remain synchronized at `badee5c`. SHADOW-006, quote-provenance hardening, and the read-only Schwab quote boundary are implemented and verified on the task branch, but the stack is not merged or armed. The branch replaces the temporary compile-time selector switch with an immutable write-once arm record tied to prerequisite proof hashes, the constitution hash, and a Shadow runtime build hash that includes the Schwab quote module. It implements canonical rank/score/identity ordering, fatal versus informational warnings, 30-second quote and multi-clock freshness gates, report/provider/quote identity, Opportunity ID and same-day duplicate controls, one global active position, regular-session/early-close/no-overnight behavior, expected-cycle and restart-downtime accounting, SPY/IWM and eligible-candidate counterfactuals, and the 10-session strategy-review gate. Active Monitor wrapper time cannot qualify copied bid/ask values as fresh. Production now defaults to one exact-host Schwab Market Data quote GET, derives executable time from the oldest provider bid/ask/quote timestamp, refreshes OAuth only through the immutable sole-`2573`-CASH guard, rejects malformed, delayed, closed, extended-hours, stale, mismatched, non-finite, and unrequested evidence, and exposes no order method. A live Sunday read proved the real Schwab response shape and correctly rejected Friday CRWV/SPY quotes as stale, closed, extended-hours evidence. Compileall, 53 focused tests, full Python discovery at 836/836, and all 216 .NET tests pass. Production-local proof keeps activation SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`, arm/policy/state absent, and the official sample at `0 / 30`. FakeBroker remains the only automated execution boundary; Schwab remains read-only and bound to the sole `2573` `CASH` account.
...
| Active branch | `codex/ARGUS-SHADOW-006-deterministic-market-validity`, stacked on the remotely backed SHADOW-005 tip and backed up through the verified read-only Schwab quote boundary. SHADOW-006 is `IMPLEMENTED_PENDING_MERGE`; SHADOW-004 remains the visual parent pending its exact truth-label acceptance. |
...
| Blocked by | Canonical integration and backup of SHADOW-004/005/006; pending SHADOW-004 WPF truth-label acceptance; a refreshed canonical workstation/Engine Host after integration; one regular-market operational proof that Schwab provides a tradable quote inside the frozen 30-second boundary; and complete non-synthetic prerequisite proof hashes. |
| Immediate Builder work | Finish Hard Chew and back up the read-only Schwab quote boundary on the task branch. After Steven is available for the one pending SHADOW-004 visual truth check, integrate the SHADOW-004/005/006 stack, refresh the workstation/Engine Host, and run the regular-market operational proof. Create the write-once arm only after every prerequisite proof passes. Keep the official sample at zero until then. |
