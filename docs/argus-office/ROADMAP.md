...
Last reconciled: 2026-07-26 on `codex/ARGUS-SHADOW-006-deterministic-market-validity`. Local and remote `master` remain synchronized at `badee5c`. SHADOW-006 is implemented, verified, committed, and remotely backed on its task branch, but it is not merged or armed. It replaces the temporary compile-time selector switch with an immutable write-once arm record tied to prerequisite proof hashes, the constitution hash, and the Shadow runtime build hash. The branch implements canonical rank/score/identity ordering, fatal versus informational warnings, 30-second quote and multi-clock freshness gates, report/provider/quote identity, Opportunity ID and same-day duplicate controls, one global active position, regular-session/early-close/no-overnight behavior, expected-cycle and restart-downtime accounting, SPY/IWM and eligible-candidate counterfactuals, and the 10-session strategy-review gate. Compileall, 107 affected-area tests, and full Python discovery at 811/811 pass. Production-local read-only proof preserves activation SHA-256 `6980D5734F3F2010D892CD1F3E29354D5DF37B193B082B18A01D8B5D485AD20C`, leaves arm/policy/state/cycle files absent, and keeps the official sample at `0 / 30`. FakeBroker remains the only automated execution boundary; Schwab remains read-only and bound to the sole `2573` `CASH` account.
...
| Blocked by | Canonical integration and backup of SHADOW-004/005/006; pending SHADOW-004 WPF truth-label acceptance; production proof that the real monitor supplies current bid/ask observations under the frozen 30-second boundary; and complete non-synthetic prerequisite proof hashes. |
| Immediate Builder work | Complete the pending visual parent check, integrate the verified SHADOW-004/005/006 stack, then run a production-local read-only operational proof. Create the write-once arm only after every prerequisite proof passes. Keep the official sample at zero until then. |
...
| Data caveat | Yahoo/Nasdaq warnings or missing timely bid/ask observations may truthfully produce zero eligible candidates. Only `CRWV` has stored minute candles; actual-data cutover remains a destructive-operation interruption gate. The frozen early-close table covers 2026-2028 and fails closed beyond it. |
