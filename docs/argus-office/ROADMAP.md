...
| Item | Current truth |
| --- | --- |
| Canonical baseline | Canonical `master` contains R032C automatic candle backfill, R034A legacy-consumer migration, DATA-002 time-normalized RVOL authority, DATA-003 breakout/reclaim setup identity, DATA-004 same-session intraday TradePlan semantics, DATA-005 account-aware allocation enforcement, DATA-005A fresh account/portfolio evidence, the directly proven A001-A003 Alpaca Paper foundation, and the DATA-006 Finviz schema-drift repair. The August 12 scheduler and artifact path completed, but its empty candidate evidence remains quarantined. |
| Active implementation | DATA-006 is complete: current Finviz `Change %`/`Float` headers are supported and required-column drift fails loudly. DATA-005A, the directly proven A001-A003 Paper foundation, DATA-005B provider-neutral allocation/Paper-research contracts, and ARGUS-BROKER-ALPACA-004 Paper engineering wiring remain complete. Candidate-bearing provider execution remains unobserved. R034 remains a separate destructive approval gate. |
...
| Immediate operational work | Adjudicate the first post-DATA-006 candidate-bearing `PAPER_TRADE_CREATED` or gate-specific legitimate `NO_TRADE`; the 22 remaining opening jobs and the next dependent Paper job are repinned to the final repair head. Continue continuous-intraday implementation so the system is not structurally limited to one 08:35 candidate supply. Present R034's exact deletion plan only when Steven is ready. |
