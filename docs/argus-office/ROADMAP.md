...
Status: `ACTIVE`; SHADOW-001 through SHADOW-014 preparation and the Schwab read-only foundations are `COMPLETE` on `master`; SHADOW-016/017, stacked CANARY-001/002, and independent CANARY-003 are `IMPLEMENTED_PENDING_MERGE`; the official sample is `ACTIVATED`, `SELECTOR_NOT_ARMED`, `OPENING_CEREMONY_UNPROVEN`, and `0 / 30`; A017 is `BLOCKED_VENDOR_CAPABILITY`; every real-order gate remains closed
...
- CANARY-001 commit `7330983` implements the nontransmitting position-invariant core: zero positions before, one exact approved positive long position while active, and zero afterward. CANARY-002 commit `f1b9042` adds a redacted immutable phase-evidence chain that preserves blocks without advancing and permits only passing PRE/ACTIVE/POST order. Both are stacked and pending integration.
- Independent CANARY-003 commit `5702ad7` implements the R-034 settled-cash and restriction gate. It requires an explicit maximum debit and cash reserve, exact approved evidence source, open pinned CASH account, fresh collection and balance clocks, independently supplied settled cash, and known-clear restriction evidence. Ordinary cash available, buying power, and liquidation value are never accepted as settled-cash substitutes. The current Schwab read-only balance model maps honestly to `SETTLED_CASH_UNAVAILABLE` and `RESTRICTIONS_UNAVAILABLE`, so it cannot pass until an official source contract supplies both.
- CANARY-001/002/003 are prerequisites, not canary authorization. A future supervised workflow must integrate and consume them and still prove broker order identity/idempotency, broker-truth reconciliation, partial-fill/cancel/restart behavior, independent shutdown/revocation, credential remediation, and Steven's explicit real-order decision.
