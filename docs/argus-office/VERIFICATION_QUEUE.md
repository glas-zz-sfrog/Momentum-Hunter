...
| --- | --- | --- | --- | --- |
| ARGUS-SERVICE-004 reboot-without-login canary | `AUTOMATED_PASS` for implementation; Python compileall, PowerShell parsing, live installed-service `PlanOnly` nonmutation proof, 75 final focused/adjacent tests, and all 987 Python tests pass. The current service remains Running/Automatic and Healthy with two terminal installation jobs, no reboot baseline, zero Shadow jobs, and transmission `UNAVAILABLE`. The actual reboot observation is `NOT_RUN`. | `USER_ACTION_REQUIRED` only for the deliberate reboot and remaining at the Windows sign-in screen through the canary window; no visual product judgment or routine approval is requested. | `COMPLETE_AND_BACKED_UP` on canonical `master` through implementation `357c974` plus Roadmap closeout. | After Codex prepares the future canary, reboot when explicitly told, do not sign in until after the stated time, then sign in and say `check`. Codex verifies new boot/service identity, Session 0, zero logged-on interactive sessions at execution, exact account binding, Healthy Engine Host, terminal receipts, zero Shadow work, and no order transmission. |
