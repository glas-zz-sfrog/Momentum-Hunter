...
| ARGUS-SHADOW-006 automatic sample selector | `AUTOMATED_PASS`; deterministic selection, immutable proof-addressed arming, multi-clock/current-quote gates, decision-cycle accounting, counterfactuals, portfolio/session rules, compileall, 107 affected-area tests, 811 full Python tests, and production-local nonmutation proof pass | `NO_STEVEN_ACTION`; implementation, commit, and feature-branch backup use standing delegation | Verified and remotely backed at `2f86e5e`; `SELECTOR_NOT_ARMED`; `0 / 30`; no policy or trade state created | Integrate after the SHADOW-004 visual truth check, then prove the production quote boundary before arming |
