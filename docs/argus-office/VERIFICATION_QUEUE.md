...
| --- | --- | --- | --- | --- |
| ARGUS-GUI-COMMAND-CENTER-001C production integration | `AUTOMATED_PASS`; corrected isolated schema-v3 WPF proof, second-eye re-review, semantic/read-model, bridge, presentation, layout, and bounded regression evidence pass. Observed truth was session `2026-08-27`, Radar `15`, projection `PARTIAL`; missing lifecycle/report evidence remained unavailable with no fallback. | `MANUAL_PENDING` for checks 1-7 below; populated-row microchart visual review is `MANUAL_NOT_YET_AVAILABLE` under check 8 because the selected authoritative evidence produced no candidate/disposition rows. | `IMPLEMENTED_PENDING_MERGE`; merge/install/startup changes are not authorized. | Confirm the accepted macro, truthful partial state, geometry-pending treatment, clean event chronology, absence of trading controls, and native-resolution readability. Do not expect example symbols or counts. |
...
1. Open `ARGUS-GUI-COMMAND-CENTER-001C-overall-1920x1080.png` at native size.
...
2. Open `ARGUS-GUI-COMMAND-CENTER-001C-accepted-vs-implementation.png` and
...
3. Confirm the visible runtime truth is session `2026-08-27`, Radar `15`, and
   projection `PARTIAL`; Accepted and Rejected must say `UNAVAILABLE`, not zero,
   because Candidate Lifecycle evidence was absent in this proof.
...
5. Confirm `HOST PARTIAL` and `PROJECTION DATA PARTIAL` are presented as
   separate facts, and System Context names the missing lifecycle/report
   evidence rather than implying healthy or complete candidate data.
6. In the worktree runtime, allow at least two five-second refreshes. Confirm
   What Changed remains a short, stable set of genuine source/lifecycle events;
   it must not accumulate repeated `Refreshed Python Engine Host status` rows
   or attach a selected symbol to generic host activity.
...
8. `MANUAL_NOT_YET_AVAILABLE`: when authoritative current evidence later
   produces ranked, Accepted, or Rejected rows, confirm each populated surface
   shows equivalent stored 2-session / 15-minute microchart context and honest
   partial/unavailable history. Do not fabricate production rows to close this
   check; automated provenance and parity tests already pass.
