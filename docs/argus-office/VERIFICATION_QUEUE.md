...
| ARGUS-SHADOW-006 automatic sample selector | `AUTOMATED_PARTIAL_PASS`; policy fingerprinting, idempotency, pre-activation behavior, manual-start rejection, audit propagation, and the compile-time methodology gate pass compileall, 93 focused tests, and 797 full Python tests; production-local proof returns `CONSTITUTION_NOT_ARMED` without creating policy/state | `NO_STEVEN_ACTION`; Steven rejected the blanket 30-minute rule and supplied the evidence-validity direction | Verified hard-disabled checkpoint on the allowed SHADOW-005 successor; `SELECTOR_NOT_ARMED`; no policy or trade state created | Implement and prove the constitution, stable ranking, multi-clock freshness, current-quote recheck, cross-report controls, and decision-cycle denominator before Trade 1 |
