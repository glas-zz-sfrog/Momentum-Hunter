...
| ARGUS-SHADOW-006 automatic sample selector | `AUTOMATED_PASS`; deterministic selection, immutable proof-addressed arming, multi-clock/current-quote gates, provider-time separation, decision-cycle accounting, counterfactuals, portfolio/session rules, compileall, 69 focused tests, 815 full Python tests, all 216 .NET tests, and production-local nonmutation/fail-closed proof pass | `NO_STEVEN_ACTION`; implementation, commit, and feature-branch backup use standing delegation | Verified on the remotely backed task branch; `SELECTOR_NOT_ARMED`; `0 / 30`; no policy or trade state created; monitor wrapper time cannot qualify as quote time | Integrate after the SHADOW-004 visual truth check, then prove a provider-timestamped production quote boundary before arming |
