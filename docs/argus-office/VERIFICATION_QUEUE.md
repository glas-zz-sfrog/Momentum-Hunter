...
| --- | --- | --- | --- | --- |
| ARGUS-CATALYST-SCORE-AUTHORITY-001 score explanation truthfulness | `AUTOMATED_PASS`; 73 focused/adjacent tests, compileall, all discovered-test coverage, historical-hash proof, and static safety scans pass. | `MANUAL_PENDING` after the corrected runtime is promoted | `IMPLEMENTED_PENDING_PROMOTION` on `codex/ARGUS-CATALYST-SCORE-AUTHORITY-001` | 1. Open BMNR or another candidate generated by the promoted score engine and open its `Why` score explanation. 2. Confirm `Scoring Version` says `momentum_score_v2`. 3. Confirm catalyst authority context states the authorized and blocked headline counts without granting points to blocked headlines. 4. Confirm the explanation does not assert `momentum_score_v1`. 5. Confirm all labels and values fit without clipping or overlap. Report the failed step only if one fails. |
