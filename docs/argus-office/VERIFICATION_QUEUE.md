...
`4bf397b2c410760f31af317a27c66e00b87fabe7`; branch backup is complete through
this closeout by normal non-force push.
...
original pixels. The frames use deterministic, visibly simulated/read-only
fixtures; they are layout and safety evidence, not provider, account, position,
or strategy truth.

1. Open `ARGUS-GUI-COMMAND-CENTER-001-overall-1920x1080.png`. Confirm Live
   Universe, Focus Candidate/Market Story, Decision/Why/Evidence, and What
   Changed form one coherent surface; the chart owns the largest region; the
   answer/reason is prominent; and the timeline remains visible.
2. Open `ARGUS-GUI-COMMAND-CENTER-001-compact-1180x820.png` at 100%. Confirm
   `READ-ONLY RESEARCH`, Positions, What Changed, `DATA DEGRADED`, Menu, and
   minimize/maximize/close are all fully visible with no horizontal clipping.
3. In that compact frame, confirm NVDA, NVIDIA Corporation, `$176.42`, `+3.2%`,
   every candidate's price/change, and `SEEN 2026-07-13` are readable; confirm
   the chart and current answer/reason remain usable.
4. Open the loading and stale frames. Confirm `UNKNOWN / HISTORY LOADING` and
   `RECLAIM READY / QUOTE STALE` are visibly separate opportunity/evidence
   states and neither pair overlaps or clips.
5. Open the historical frame. Confirm the amber selected timeline row, separate
   amber `HISTORICAL EVIDENCE` block, stable identity/source/time, preserved
   current answer above it, and `Return to Current` prevent old evidence from
   masquerading as current truth.
6. Open the Positions/health frame. Confirm Diagnostics is compact and truthful,
   Positions says `POSITIONS — READ-ONLY` and `POSITION DATA UNAVAILABLE`, and
   no fallback position or broker/order control appears.
7. If running the task-worktree build, open Menu. Confirm Current, Replay, and
   Review remain reachable, while Pause/Resume monitoring, Run Scan, service
   control, simulation, and trading actions are absent from the Command Center.
8. Report the exact failed check number and screenshot/app element if any text
   clips, overlaps, looks misleading, or fails to remain visibly read-only.

Do not merge or install from this check. A pass only records visual acceptance;
Producer-001C must also be terminal, its mandatory ZIP must exist, the frozen
boundary must be released, and a separate post-freeze merge/install action must
be authorized.
...
1. Select NVDA, SHOP, ZETA, SPY, and IWM and confirm each has meaningful recent
   1m history rather than one or two proof bars.
2. Switch to 5m and 15m and confirm each interval is a deterministic aggregation
   of the same Schwab 1m source with no unexplained gaps.
3. Switch to Daily and confirm a useful historical series appears from the
   separate Schwab daily store.
4. Confirm each interval names Schwab `/pricehistory`, the latest provider bar
   time, receipt time/age, gap count, and stale/insufficient state truthfully.
5. Confirm no Yahoo, CRWV legacy, simulated, quote-derived, or cross-timeframe
   fallback candle appears in a Schwab series.
