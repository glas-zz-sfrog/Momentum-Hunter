...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-R036-staged-candle-preview-host-7cbc2cb` | Branch tip containing the R036 verification closeout above `f030051` | Pending ordinary non-force feature-branch backup | No | `ACTIVE / IMPLEMENTED_PENDING_MERGE` | Canonical backend-only successor exposing the hash-verified inactive R035 stage through one bounded idempotent Engine Host command, with strict payload sanitization and no WPF/provider/account/write/order path. | Back up the feature branch, then merge after R035 only when the frozen SHADOW-016 operational audit releases `master`. |
