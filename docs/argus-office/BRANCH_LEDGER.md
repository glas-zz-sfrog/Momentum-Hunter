...
`codex/ARGUS-INTEGRATION-002-release-gate`. SCHWAB-004/004A and CANARY-015/016/017
...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-SCHWAB-004-account-shape-evidence` | `0f26938` plus governance closeout | Yes after closeout push | No | `ACTIVE / IMPLEMENTED_PENDING_MERGE` | One stacked successor containing SCHWAB-004/004A account evidence, CANARY-015 identity binding, CANARY-016's bounded nontransmitting worker, and CANARY-017's exact-interpreter lifecycle supervisor. The supervisor launches one local process, persists immutable identity/running/stop/acknowledgement/stopped evidence, and writes a sanitized result that remains blocked only on provider revocation. It has no Schwab, credential, account-query, broker, order, retry, termination, signal, or execution path. Compileall, 10 focused, 112 adjacent CANARY, 443 Schwab, 1,290 full Python, and 216 .NET tests pass. | Preserve remotely. Use CANARY-015/016/017 as the identity/worker/supervision boundary for a future provider-coupled worker, never as provider or execution proof. After SHADOW-014 releases `master`, regenerate the final stack manifest including CANARY-015/016/017 and integrate only after all release gates pass. |
