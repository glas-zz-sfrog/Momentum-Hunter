...
share the one active stacked successor. The linear chain preserves
...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-SCHWAB-004-account-shape-evidence` | `87a811a` plus governance closeout | Yes after closeout push | No | `ACTIVE / IMPLEMENTED_PENDING_MERGE` | One stacked successor containing SCHWAB-004/004A account evidence, CANARY-015 identity binding, CANARY-016's bounded nontransmitting worker, CANARY-017's exact-interpreter lifecycle supervisor, CANARY-018's read-only lifecycle-package verifier, CANARY-019's frozen V3 stack-integrity policy, and CANARY-020's exact final release gate. CANARY-020 requires the exact reviewed clean branch and synchronized upstream/base, frozen source, exact path union, reviewed runtime/test/tool blobs, safe paths/content, and a valid 17-component V3 manifest. The real branch passes with zero findings at `87a811a`; execution permit is false and order transmission unavailable. It has no Schwab, credential, account-query, broker, order, retry, termination, signal, or execution path. | Preserve remotely. After SHADOW-014 releases `master`, rerun `tools/verify_canary_stack_release.py` and the complete suites at the final synchronized HEAD; integrate only if the exact gate passes. Never treat Git/content proof as provider or execution evidence. |
