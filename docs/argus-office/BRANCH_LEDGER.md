...
`codex/ARGUS-INTEGRATION-002-release-gate`. SCHWAB-004/004A and CANARY-015/016/017/018
...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-SCHWAB-004-account-shape-evidence` | `d8f6a19` plus governance closeout | Yes after closeout push | No | `ACTIVE / IMPLEMENTED_PENDING_MERGE` | One stacked successor containing SCHWAB-004/004A account evidence, CANARY-015 identity binding, CANARY-016's bounded nontransmitting worker, CANARY-017's exact-interpreter lifecycle supervisor, and CANARY-018's read-only lifecycle-package verifier. The supervisor launches one local process and persists immutable identity/running/stop/acknowledgement/stopped evidence. The verifier requires externally pinned identities, reopens the supporting records, recomputes cross-record binding, and blocks summary-only, tampered, stale, or drifted packages. It has no Schwab, credential, account-query, broker, order, retry, termination, signal, or execution path. Compileall, 10 focused, 242 adjacent CANARY, 453 Schwab, 1,300 full Python, and 216 .NET tests pass. | Preserve remotely. Use CANARY-015/016/017/018 as the identity/worker/supervision/revalidation boundary for a future provider-coupled worker, never as provider or execution proof. After SHADOW-014 releases `master`, regenerate the final stack manifest including CANARY-015/016/017/018 and integrate only after all release gates pass. |
