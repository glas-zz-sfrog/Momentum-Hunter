...
`codex/ARGUS-INTEGRATION-002-release-gate`. SCHWAB-004/004A and CANARY-015/016/017/018/019/020
share the frozen foundation-release successor. R035/R036 share the one active product
successor stacked on that branch. The linear chain preserves
byte-equivalent runtime/test content while the original branches remain remotely
backed source history. CANARY-020's committed non-mutating final gate now rechecks
the exact branch and upstream, source refs, ancestry, exact path union, byte identity,
risky paths, secret signatures, and the 17-component V3 manifest in one decision. It
remains pending integration while `master` is frozen for the one-time SHADOW-014
opening proof. No included path contains a real-order action or grants canary
authority.
...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-R035-staged-schwab-chart-preview-host` | `60ccd37` plus R035/R036 closeout | Yes after closeout push | No | `ACTIVE / IMPLEMENTED_PENDING_MERGE / MANUAL_PENDING` | R035 adds the idempotent Engine Host read command over hash-verified inactive staging. R036 adds an explicit session-only WPF consumer that defaults to stored data, independently validates preview identity and safety flags, never falls back between sources, and labels preview state as inactive/nontransmitting. Python compileall, 43 bounded host/staging tests, 19 focused and 222 total .NET tests, zero-warning Release build, and two responsive offscreen proofs pass. No provider, credential/account, write/delete, active-source mutation, broker, or order path was added. | Preserve as the sole product successor. Complete Steven's exact R036 visual checklist, then integrate the CANARY-020 foundation after SHADOW-014 releases it and fast-forward this ancestry-compatible product successor only after final tests remain exact. Actual candle cutover remains destructive-gated. |
| `codex/ARGUS-R030-staged-schwab-chart-preview-host` | `448f05e` | Yes | No | `DO_NOT_USE` | Preserved pointer to the same verified R035 work before the existing R030-R034 task-ID collision was discovered. No content was discarded or rewritten. | Do not continue, merge, or use this branch name. Use `codex/ARGUS-R035-staged-schwab-chart-preview-host`; branch deletion requires separate Steven approval. |
| `codex/ARGUS-SCHWAB-004-account-shape-evidence` | `ec32b98` | Yes | No | `FOUNDATION_RELEASE / IMPLEMENTED_PENDING_MERGE` | One frozen foundation successor containing SCHWAB-004/004A account evidence, CANARY-015 identity binding, CANARY-016's bounded nontransmitting worker, CANARY-017's exact-interpreter lifecycle supervisor, CANARY-018's read-only package verifier, CANARY-019's frozen V3 stack-integrity policy, and CANARY-020's exact final release gate. CANARY-020 requires the exact reviewed clean branch and synchronized upstream/base, frozen source, exact path union, reviewed runtime/test/tool blobs, safe paths/content, and a valid 17-component V3 manifest. The real branch passes with zero findings; execution permit is false and order transmission unavailable. It has no Schwab, credential, account-query, broker, order, retry, termination, signal, or execution path. | Keep frozen and remotely preserved. After SHADOW-014 releases `master`, rerun `tools/verify_canary_stack_release.py` and the complete suites at the final synchronized HEAD; integrate only if the exact gate passes. Never rewrite it to absorb R035 or treat Git/content proof as provider or execution evidence. |
...
| `codex/ARGUS-CANARY-013-process-evidence-chain` | `9f15046` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Adds create-exclusive persistence for one exact CANARY-012 process target and a sequential immutable hash-linked observation chain. Conflicts, gaps, tamper, identity/chronology drift, stopped-to-running contradiction, partial files, symlinks, and concurrent writers fail closed; it performs no process, credential, network, or broker mutation. | Preserve remotely until SHADOW-014 releases `master`; later bind this store to the exact future broker-worker launch identity and combine it with separately approved provider-revocation evidence in a physical stop drill. |
| `codex/ARGUS-CANARY-012-readonly-process-observer` | `b45cf81` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Adds a Windows limited-information process observer with creation-time/executable commitment identity, PID-reuse handling, fail-closed unavailable states, and CANARY-005 mapping. It performs no process, credential, network, or broker mutation. | Preserve as CANARY-013's parent; use the immutable successor chain rather than persisting observer results through a parallel path. |
| `codex/ARGUS-CANARY-011-official-order-schema-evidence` | `2446582` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Adds an offline, credential-sanitizing inspector and read-only CLI for an official Schwab OpenAPI export. It hashes the source, follows local references only, verifies the GET order collection/detail schema, and names unavailable reconciliation fields without a provider call or broker action. | Preserve remotely until SHADOW-014 releases `master`, then inspect a locally exported official specification. Implement a GET-only provider mapper only for fields that the inspected official schema proves. |
| `codex/ARGUS-CANARY-010-stack-integrity-manifest` | `ea6ee3e` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Adds a nonauthorizing content-addressed manifest/verifier for the exact CANARY-001 through CANARY-010 runtime stack, build identity, file hashes, path containment, static safety boundary, and fixed false authority flags. | Preserve remotely; after SHADOW-014 releases `master`, use a fresh final-HEAD manifest plus full tests during clean integration. Never treat it as provider evidence or execution authority. |
| `codex/ARGUS-CANARY-009-order-contract-emulator` | `0a8f038` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Adds a sanitized, deterministic, nonnetwork order-contract emulator for acknowledgement loss, partial fills, cancel/fill races, terminal states, restart recovery, and tamper rejection. It cannot accept credentials, call Schwab, or act on an order. | Preserve remotely; integrate only after the SHADOW-014 baseline is released. Use it for failure-state proof, never as provider evidence or execution authority. |
| `codex/ARGUS-CANARY-008-manual-decision-intent` | `3d070e8` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Adds an unauthenticated, nontransmitting, write-once exact approve/decline intent contract over one valid receipt and bounded BUY LIMIT order. It creates no Steven decision or broker authority. | Preserve remotely; integrate only after the SHADOW-014 baseline is released. A later physically verified confirmation channel must remain a consequential Steven gate. |
| `codex/ARGUS-CANARY-007-immutable-preflight-receipt` | `a35bfc9` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Adds a redacted write-once, expiring receipt for the exact CANARY-006 evidence set and revalidates source-chain continuity during inspection; it has no network, credential, process, or order action. | Preserve remotely; integrate only after the SHADOW-014 baseline is released, then use it only after unavailable official provider/physical evidence is obtained. |
| `codex/ARGUS-CANARY-006-preflight-composition` | `a6c2aeb` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Composes CANARY-001 through CANARY-005 into one fresh, identity-bound, fail-closed preflight that can only await Steven's separate real-order decision; it has no network, credential, process, or order action. | Preserve remotely; integrate only after the SHADOW-014 baseline is released, then obtain the currently unavailable official funding/order/stop evidence without weakening the contracts. |
| `codex/ARGUS-CANARY-005-independent-stop-evidence` | `73b939a` plus governance closeout | Yes | No | `IMPLEMENTED_PENDING_MERGE` | Adds a write-once external stop latch and fail-closed shutdown/revocation drill evidence evaluator with no process, credential, network, or order mutation. | Preserve remotely; integrate only after the frozen baseline is released, then implement and physically prove separate OS/provider actions under consequential approval. |
...
| `codex/ARGUS-SHADOW-017-evidence-checkpoints` | `4858d73` | Yes | No | `SUPERSEDED` | Adds automatic immutable 5/10/20/30 evidence checkpoints on top of SHADOW-016. | Preserve as source history; its exact runtime/test content is carried by ARGUS-INTEGRATION-001. |
| `codex/ARGUS-SHADOW-016-paper-reconciliation-evidence` | `a2c3637` | Yes | No | `SUPERSEDED` | Adds immutable manual paperMoney reconciliation and a read-only fill-model-error audit. | Preserve as source history and SHADOW-017 parent; its exact runtime/test content is carried by ARGUS-INTEGRATION-001. |
| `codex/ARGUS-SHADOW-015-opening-failure-rehearsals` | `cd828ac` | Yes | No | `SUPERSEDED` | Preserves opening-failure rehearsal evidence and nontransmitting candle-source staging/cutover preflight without performing destructive cutover. | Preserve as source history; its exact runtime/test content is carried by ARGUS-INTEGRATION-001. |
