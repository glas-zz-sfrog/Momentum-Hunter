...
Local `master` at `badee5c` is the canonical integrated product baseline and is
synchronized with `origin/master`. It contains the Python automation/simulation
foundation, R004-R029 workstation work, ARGUS-SHADOW-001/002/003, and
SCHWAB-001/002/002A/003.

`codex/ARGUS-SHADOW-004-official-sample-activation` is the active visual parent at
`375da59`. Its write-once activation boundary is automatically verified and one WPF
truth-label acceptance item remains before integration. The production-local sample is
`ACTIVATED`, its selector is not armed, and it has zero completed trades.
...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-SHADOW-006-deterministic-market-validity` | Current verified task tip based on SHADOW-005 | Yes; feature branch only | No | `PUSHED_FEATURE_BRANCH / IMPLEMENTED_PENDING_MERGE` | Implements the deterministic Official Shadow V1 selection, market-validity, deduplication, portfolio, session, decision-cycle, counterfactual, diversity, and immutable arm boundaries. Production remains unarmed and `0 / 30`. | Integrate only with the accepted SHADOW-004 visual parent, then run production quote-boundary proof before arming. |
| `codex/ARGUS-SHADOW-005-prospective-evidence-handoff` | `df1c78a` plus backup-status closeout | Yes | No | `PUSHED_FEATURE_BRANCH / SUPERSEDED_BY_SHADOW_006` | Contains the verified capture-to-report handoff and the earlier hard-disabled automatic-selector foundation. | Preserve as parent history; review and integrate the SHADOW-006 successor rather than continuing here. |
