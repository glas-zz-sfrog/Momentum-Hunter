...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-SCHWAB-003-readonly-account-discovery` | `3691967` plus current binding closeout | No | No | `ACTIVE` | Adds exact GET-only discovery, live CASH validation, immutable DPAPI binding to the sole `2573` `INDIVIDUAL_CASH` account, and bound-refresh revalidation; every transmitting capability remains unavailable. | Post-binding compileall, 123 bounded tests, and exact tracked-value scan pass; complete fast-forward/remote-divergence preflight, then integrate and non-force-push under standing delegation. |
