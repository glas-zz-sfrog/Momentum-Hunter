...
share the frozen foundation-release successor. R035 is the one active product
...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-R035-staged-schwab-chart-preview-host` | `60ccd37` plus governance closeout | Yes after closeout push | No | `ACTIVE / IMPLEMENTED_PENDING_MERGE` | Adds one Engine Host read command over the existing hash-verified inactive Schwab staged-chart service. The command is idempotent, validates exact preview/inactive/nontransmitting lineage, sanitizes invalid evidence, and performs no provider call, credential/account access, file write/delete, active chart switch, UI render, broker action, or transmission. Compileall, 43 focused, 94 bounded, 1,317 full Python, and 216 .NET tests pass. | Preserve as the sole product successor. Integrate the CANARY-020 foundation first after SHADOW-014 releases it; then rebase-free fast-forward R035 only after ancestry and final tests remain exact. Any later WPF consumer requires separate visual proof, and actual candle cutover remains destructive-gated. |
