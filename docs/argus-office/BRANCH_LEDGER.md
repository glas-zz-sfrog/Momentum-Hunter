...
- ARGUS-SHADOW-004 started from synchronized `master` at `badee5c`. It has not been committed, merged, or pushed. Its local activation evidence is ignored generated state and is not a Git artifact.
...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-SHADOW-004-official-sample-activation` | `badee5c` plus verified working-tree implementation | No | No | `ACTIVE` | Adds a write-once CLI-only official-sample activation boundary, restart loading, direct-bypass rejection, post-activation evidence timing, and truthful active-empty WPF status. The ignored local sample is active at `0 / 30`; no trade state or transmitting method exists. | Record Steven's one visual acceptance item, commit, fast-forward into `master`, non-force-push, then wait for fresh post-activation evidence before trade 1. |
