...
| --- | --- | --- | --- | --- | --- | --- |
| `codex/ARGUS-CANARY-013-process-evidence-chain` | `9f15046` plus governance closeout | Yes | No | `ACTIVE / IMPLEMENTED_PENDING_MERGE` | Adds create-exclusive persistence for one exact CANARY-012 process target and a sequential immutable hash-linked observation chain. Conflicts, gaps, tamper, identity/chronology drift, stopped-to-running contradiction, partial files, symlinks, and concurrent writers fail closed; it performs no process, credential, network, or broker mutation. | Preserve remotely until SHADOW-014 releases `master`; later bind this store to the exact future broker-worker launch identity and combine it with separately approved provider-revocation evidence in a physical stop drill. |
