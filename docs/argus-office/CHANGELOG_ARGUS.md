...
## Unreleased
- Hardened BREAKOUT-002A outcome integrity after reproducing two fail-open paths: historical-replay bars can no longer complete a prospective event, and revision chronology cannot move backward in either the builder or persisted ledger. Compileall, 32 focused tests, 136 adjacent regressions, and all 1,651 Python tests pass; the module remains dormant and research-only.
- Added BREAKOUT-002A's dormant sequential outcome/cohort contract. It binds exact BREAKOUT-001 event-chain prefixes to immutable 5/15/30/60-minute revisions, preserves pending/gap/missing/correction/session-boundary evidence, separates replay from prospective denominators, and withholds every conclusion until a later frozen cohort adjudication. Compileall, 29 focused tests, 136 adjacent regressions, and all 1,648 Python tests pass; canonical runtime remains unchanged.
