...
## Unreleased
- Added SHADOW-025F's dormant runtime writer session. One current Engine Host claim now owns a process-lifetime OS lease while appending source admissions, with exact host/PID/build/config/topology revalidation and finite replacement behavior. Source-admission and ledger schema v2 bind configuration and evidence-program identity under a complete ordered-ledger fingerprint. A self-review race fix prevents concurrent reuse of one single-use session. Compileall, 43 focused, 248 adjacent, and all 1,783 Python tests pass; no installed runtime path changed.
- Added SHADOW-025E explicit-path append-only persistence for runtime source admissions. The shared finite cross-process lease preserves old event-cycle behavior while the admission store validates complete plan lineage, exact replay, atomic replacement, tamper resistance, simultaneous Windows writers, and process-exit recovery. All 1,766 Python tests pass; no installed root or runtime path changed.
- Added SHADOW-025D's dormant runtime topology contract. Python Engine Host is the sole online artifact owner, Automation Service remains supervisor-only, WPF stays behind versioned Engine Host snapshots, and offline review is read-only. Program/configuration-scoped paths and current host/PID/build claims prevent cross-sample evidence mixing and stale-writer reuse. All 1,757 Python tests pass; no installed root or runtime path changed.
- Added SHADOW-025C runtime-source admission to the dormant continuous decision path. Each new plan version admits one exact setup-bound candidate event or one exact successor plan; canonical ledger membership, predecessor chronology, materiality, policy, setup, and source identity fail closed. Context changes and candidate refreshes cannot double-fire parallel cycles, and the adapter has no provider, broker, persistence, or installed-runtime capability. All 1,734 Python tests pass.
- Added finite cross-process ownership to SHADOW-025's dormant event-cycle ledger. One deterministic sidecar lease now covers full read/validate/append/atomic-write transactions and direct saves while preserving same-thread reentrancy. Windows-spawn tests prove simultaneous writers, finite timeout, recovery after normal release, and recovery after process exit. Compileall, 42 focused, 98 direct-contract, 268 combined-continuous, 130 adjacent, and all 1,717 Python tests pass; no runtime importer or installed path changed.
- Reconciled the dormant continuous evidence, hardened PLAN-002A, BREAKOUT-002A, and SHADOW-025A contracts on one offline branch. The combined audit updated stale synthetic PLAN fixtures and prevents a rehashed event-cycle decision from contradicting its exact supplied plan authority. Compileall, 94 focused, 264 combined, 130 adjacent, and all 1,713 Python tests pass; canonical runtime remains untouched.
- Hardened PLAN-002A after reproducing three fail-open evidence paths: decisions now retain plan/risk/allocation authority states and re-derive blockers, plan and manual-override chronology cannot move backward, and historical policy profiles cannot gain risk-review authority. Compileall, 24 focused tests, 191 adjacent regressions, and all 1,643 Python tests pass; the module remains dormant and uninstalled.
- Added PLAN-002A's dormant immutable continuous-plan version and decision-binding contract. It binds DATA-004 plans to exact opportunity/setup, regime, macro-event, catalyst, RVOL, source-clock, policy, predecessor, risk, and allocation identities; blocks research-only evidence; rejects forged authority, tampering, stale/blocking context, chain branching, and reused manual-override decisions. Compileall, 19 focused tests, 210 adjacent regressions, and all 1,638 Python tests pass; production PLAN-002 wiring remains gated and canonical runtime is unchanged.
...
- Reconciled A003 source identity: `7ccbad5` is the original harness,
  ...
  exact implementation chain before DATA-005B reconciliation.
...
- Backed up secure Canary Paper onboarding at `39576d9`. The active credentials remain outside Git under lane-specific CurrentUser DPAPI; the strategy-research slot remains empty and disabled.
...
- Removed every executable dependency on the TradePlan report's `$500` reference quantity. Simulation and Shadow now freeze allocation, policy, account-context, and quantity fingerprints between Risk Governor and FakeBroker evidence. All 1,296 Python and all 251 .NET tests pass; no provider/account fetch, live broker method, UI, scoring/rank/alert/RVOL, DATA-004 setup/timing, service/scheduler, schema/package, credential, capture, generated-report, or historical-evidence behavior changed.
- Added DATA-004's versioned same-session `INTRADAY` TradePlan contract for `OPENING_BREAKOUT`, `CONTINUATION_BREAKOUT`, `PULLBACK`, `RECLAIM`, and authority-proven catalyst setups. Opening is one producer, not the horizon model.
- Bound setup-aware validity, expiry, stop/target rules, forced-flat time, lifecycle, source evidence, predecessor/replacement identity, plan ID, and fingerprint through reports, Risk Governor, Active Monitor, simulation, and independent Shadow validation. A missed breakout is immutable and a reclaim requires a new successor plan. Compileall, all 1,271 Python tests, and all 251 .NET tests pass; scoring, rank, alerts, RVOL, UI, capture/service/scheduler, providers, accounts/orders, transmission, schemas, packages, credentials, raw captures, and generated data remain unchanged.
...
- Fast-forwarded DATA-002 feature commit `876eb2e`, pushed the feature and canonical branches normally, refreshed the guarded Engine Host identity, and repinned all 25 future opening jobs to the final synchronized closeout head with Monday pending, zero Shadow jobs, and transmission unavailable.
- Added R034A's strict read-only adapter for terminal, price-history-backed Schwab minute partitions and migrated outcome, evidence-health, data-quality, read-model, research, Daily-symbol, source-registry, and SQLite reporting defaults away from the retired CRWV cache.
- Removed the outcome updater's Yahoo minute fetch path, blocked recreation of the exact production `opportunity-minute-bars.json`, retained explicit fixture support, and versioned affected report producers so prospective output names its new source contract.
- Added a nonmutating R034 verifier that proves the exact 710-bar CRWV JSON/710-row SQLite identity, canonical Schwab-store health, source references, external archive destination, rollback conditions, and input nonmutation. It contains no archive/delete/provider/account/order/database-write capability; R034 remains separately approval-gated.
- Recorded Steven's 2026-08-06 acceptance of R032C's readable loading and intentional fail-closed visual states, integrated the stacked R032C/R034A release, and proved installed automatic history loading with previously uncached QQQ: 180 stored candles on 1m, 5m, 15m, and Daily, one expected account, no position/order request, and transmission unavailable.
...
- Added the R032 bounded Schwab one-minute collector and a separate `schwab-candles-v1` evidence store. It selects exact-date Hunter, selected/active, SPY, and IWM symbols under a ten-symbol ceiling; preserves every stream/history version; makes price history canonical; and exposes correction, gap, stale, and unreconciled states.
- Added fail-closed candidate identity checks, cross-process writer leasing, atomic daily/symbol partitions, semantic version hashes, strict partition validation, source nonmixing, current-session health, bounded history retries, and write-once run results. The collector is manual and has no WPF, Engine Host, Shadow, scoring/readiness, broker/order, service, scheduler, or legacy-store integration.
- Preserved the final exact-code extended-hours proof outside Git. Schwab delivered SPY, IWM, NVDA, SHOP, and ZETA; stream and price-history reconciliation passed, while the overall result correctly remained `PARTIAL` for visible sparse-market gaps. Positions/orders were not requested and transmission remained `UNAVAILABLE`.
...
- Preserved Monday's successful 2026-08-03 capture-only evidence and released the temporary integration freeze without arming Shadow or enabling order transmission. The capture completed on its first attempt with clock proof, four candidates, score breakdowns, and TradePlan artifacts; the analytical output remains planning-only pending quote, catalyst, setup, volume, sizing, and horizon repairs.
- Reconciled SHADOW-024 onto the post-Monday baseline without rewriting its frozen branch; compileall, 17 focused packet tests, 225 adjacent lifecycle tests, all 1,051 Python tests, diff check, protected-path review, and forbidden-capability scans pass.
- Released the temporary integration freeze after Monday's terminal capture evidence, preserved that completed receipt, and repinned the remaining 29 ordinary opening jobs to the final synchronized canonical head without creating any Shadow job or transmission capability.
- Added bounded recovery for transient Windows access-denied or sharing-violation failures while atomically replacing the unattended supervisor state receipt. Persistent locks and unrelated filesystem failures still fail closed, and the prior valid state remains intact.
- Reproduced the original failure from a concurrent read, proved real Windows lock recovery in 0.265 seconds, and passed Python compileall, 26 focused supervisor tests, 74 affected automation/capture tests, and all 1,019 Python tests. After installed reload, twelve live-receipt locks caused no process restart or Application error; Monday remained pending and the manifest hash stayed unchanged. Job timing, capture/provider behavior, Shadow, accounts, broker/orders, and transmission remain unchanged.
...
- Closed the reboot-without-login reliability gate with an exact-time forced restart at 2026-07-31 16:39 Central. Windows produced a new kernel boot and service instance; the service ran in Session 0 with zero interactive sessions; the account/DPAPI canary and exact-response Codex probe completed; all 30 opening captures remained pending; and no position/order request, Shadow job, or transmission capability existed.
- Added approval-first five-minute reboot tooling through `40729c9` and `e24feed`. It announces one exact target before restart, requires both canary receipts to become pending, preserves the complete opening schedule, fails closed below three minutes, records a launch receipt, and uses a forced restart so applications cannot silently cancel the reboot. Earlier failed/invalid attempts remain preserved without reclassification.
- Repaired the reboot-canary planner after the first physical reboot exposed an uppercase job-ID timestamp that the production supervisor correctly rejected. The failed attempt is preserved and did not create a canary receipt, Codex receipt, account/position/order request, or transmission path.
- Added production-parser validation for generated reboot manifests. ARGUS-SERVICE-005 `becbd6d` passes Python compileall, 35 focused service/reboot tests, and all 988 Python tests; the installed service recovered and the later final repeat recorded above passes.
...
- Passed Python compileall, PowerShell parsing, 75 final focused/adjacent tests, all 987 Python tests, and a live redacted Windows-session census for the original ARGUS-SERVICE-004 implementation. At that stage, the deliberate physical reboot observation was still pending.
...
- Verified the SYSTEM-owned 08:15 wake task is Ready with service-account logon, wake enabled, and missed-start backfill disabled. Windows Time reports synchronized stratum 2 against NIST; the reboot canary has now run once unsuccessfully before execution and requires one repaired repeat.
